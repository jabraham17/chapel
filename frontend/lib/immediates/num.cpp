...
static int snprint_float_val(char* buf, size_t max, double val, bool hex) {
  if (std::isfinite(val)) {
    int nc = 0;
    if (std::signbit(val))
      nc = snprintf(buf, max, "-%g", -val);
    else
      nc = snprintf(buf, max, "%g", val);
...
static int snprint_imag_val(char* str, size_t max, double val, bool hex) {
  int numchars = snprint_float_val(str, max, val, hex);
  strncat(str, "i", max - numchars);
  return numchars + 1;
}

static int snprint_complex_val(char* str, size_t max, double real, double imm) {
  int numchars = 0;
...
int snprint_imm(char* str, size_t max, const Immediate& imm) {
  int res = -1;
  switch (imm.const_kind) {
    case NUM_KIND_NONE: break;
    case NUM_KIND_BOOL: {
      // Since I'm using uints to store the bools, should cast to the
      // same to be consistent.
      res = snprintf(str, max, "%u", (unsigned)imm.bool_value());
      break;
    }
    case NUM_KIND_UINT: {
      switch (imm.num_index) {
        case INT_SIZE_8:
          res = snprintf(str, max, "%u", (unsigned)imm.v_uint8);
          break;
        case INT_SIZE_16:
          res = snprintf(str, max, "%u", (unsigned)imm.v_uint16);
          break;
        case INT_SIZE_32:
          res = snprintf(str, max, "%u", (unsigned)imm.v_uint32);
          break;
        case INT_SIZE_64:
          res = snprintf(str, max, "%" PRIu64, imm.v_uint64);
          break;
    ...
    case NUM_KIND_INT: {
      switch (imm.num_index) {
        case INT_SIZE_8: res = snprintf(str, max, "%d", imm.v_int8); break;
        case INT_SIZE_16: res = snprintf(str, max, "%d", imm.v_int16); break;
        case INT_SIZE_32:
          res = snprintf(str, max, "%" PRId32, imm.v_int32);
          break;
        case INT_SIZE_64:
          res = snprintf(str, max, "%" PRId64, imm.v_int64);
          break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    ...
    case NUM_KIND_IMAG:
      switch (imm.num_index) {
        case FLOAT_SIZE_32:
          res = snprint_float_val(str, max, imm.v_float32, true);
          break;
        case FLOAT_SIZE_64:
          res = snprint_float_val(str, max, imm.v_float64, true);
          break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    case NUM_KIND_COMPLEX:
      switch (imm.num_index) {
        case COMPLEX_SIZE_64:
          res =
            snprint_complex_val(str, max, imm.v_complex64.r, imm.v_complex64.i);
          break;
        case COMPLEX_SIZE_128:
          res = snprint_complex_val(
            str, max, imm.v_complex128.r, imm.v_complex128.i);
          break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
...
int fprint_imm(FILE* fp, const Immediate& imm, bool showType) {
  int res = -1;
  switch (imm.const_kind) {
    case NUM_KIND_NONE:
    ...
    case NUM_KIND_COMPLEX:
      switch (imm.num_index) {
        case COMPLEX_SIZE_64: {
          char str[160];
          res = snprint_complex_val(
            str, sizeof(str), imm.v_complex64.r, imm.v_complex64.i);
          fputs(str, fp);
        ...
        case COMPLEX_SIZE_128: {
          char str[160];
          res = snprint_complex_val(
            str, sizeof(str), imm.v_complex128.r, imm.v_complex128.i);
          fputs(str, fp);
...
#define DO_FOLD(_op, add, sub)                                               \
  switch (imm->const_kind) {                                                 \
    case NUM_KIND_NONE: break;                                               \
    case NUM_KIND_BOOL: {                                                    \
      imm->v_bool = im1.bool_value() _op im2.bool_value();                   \
      break;                                                                 \
    }                                                                        \
    case NUM_KIND_UINT: {                                                    \
      switch (imm->num_index) {                                              \
        case INT_SIZE_8: imm->v_uint8 = im1.v_uint8 _op im2.v_uint8; break;  \
        case INT_SIZE_16: imm->v_uint16 = im1.v_uint16 _op im2.v_uint16;     \
          break;                                                             \
        case INT_SIZE_32: imm->v_uint32 = im1.v_uint32 _op im2.v_uint32;     \
          break;                                                             \
        case INT_SIZE_64: imm->v_uint64 = im1.v_uint64 _op im2.v_uint64;     \
          break;                                                             \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement"); \
      }                                                                      \
      break;                                                                 \
    ...
    case NUM_KIND_INT: {                                                     \
      switch (imm->num_index) {                                              \
        case INT_SIZE_8: imm->v_int8 = im1.v_int8 _op im2.v_int8; break;     \
        case INT_SIZE_16: imm->v_int16 = im1.v_int16 _op im2.v_int16; break; \
        case INT_SIZE_32: imm->v_int32 = im1.v_int32 _op im2.v_int32; break; \
        case INT_SIZE_64: {                                                  \
  ...
  switch (imm->const_kind) {                                                 \
    ...
    case NUM_KIND_COMPLEX:                                                   \
      switch (imm->num_index) {                                              \
        ...
        case COMPLEX_SIZE_128: {                                             \
          imm->v_complex128 =                                                \
            complexPow128(im1.v_complex128, im2.v_complex128);               \
          break;                                                             \
        }                                                                    \
      }                                                                      \
      break;                                                                 \
  ...
  switch (imm->const_kind) {                                                 \
    case NUM_KIND_COMPLEX:                                                   \
      switch (imm->num_index) {                                              \
        ...
        case COMPLEX_SIZE_128: {                                             \
          imm->v_complex128 =                                                \
            complexDiv128(im1.v_complex128, im2.v_complex128);               \
          break;                                                             \
        }                                                                    \
      }                                                                      \
      break;                                                                 \
    default: DO_FOLD(/, false, false); break;                                \
  }
  ...
  switch (imm->const_kind) {                                                 \
    case NUM_KIND_COMPLEX:                                                   \
      switch (imm->num_index) {                                              \
        ...
        case COMPLEX_SIZE_128: {                                             \
          imm->v_complex128 =                                                \
            complexMul128(im1.v_complex128, im2.v_complex128);               \
          break;                                                             \
        }                                                                    \
      }                                                                      \
      break;                                                                 \
...
static void doFoldSqrt(chpl::Context* context,
  ...
  switch (imm->const_kind) {
    case NUM_KIND_NONE: break;
    case NUM_KIND_BOOL: {
      imm->v_bool = im1.bool_value();
      break;
    }
    case NUM_KIND_UINT: {
      switch (imm->num_index) {
        case INT_SIZE_8: imm->v_uint8 = (uint8_t)sqrt(im1.v_uint8); break;
        case INT_SIZE_16: imm->v_uint16 = (uint16_t)sqrt(im1.v_uint16); break;
        case INT_SIZE_32: imm->v_uint32 = (uint32_t)sqrt(im1.v_uint32); break;
        case INT_SIZE_64: imm->v_uint64 = (uint64_t)sqrt(im1.v_uint64); break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    }
    case NUM_KIND_INT: {
      switch (imm->num_index) {
        case INT_SIZE_8: imm->v_int8 = (int8_t)sqrt(im1.v_int8); break;
        case INT_SIZE_16: imm->v_int16 = (int16_t)sqrt(im1.v_int16); break;
        case INT_SIZE_32: imm->v_int32 = (int32_t)sqrt(im1.v_int32); break;
        case INT_SIZE_64: imm->v_int64 = (int64_t)sqrt(im1.v_int64); break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    }
    ...
    case NUM_KIND_IMAG:
      switch (imm->num_index) {
        case FLOAT_SIZE_32: imm->v_float32 = sqrt(im1.v_float32); break;
        case FLOAT_SIZE_64: imm->v_float64 = sqrt(im1.v_float64); break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    case NUM_KIND_COMPLEX:
      switch (imm->num_index) {
        case COMPLEX_SIZE_64:
          imm->v_complex64 = complexSqrt64(im1.v_complex64);
          break;
        case COMPLEX_SIZE_128:
          imm->v_complex128 = complexSqrt128(im1.v_complex128);
          break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
...
static float complexAbs64(complex64 x) { return sqrt(x.r * x.r + x.i * x.i); }
  ...
  switch (im1.const_kind) {
    ...
    case NUM_KIND_UINT: {
      switch (im1.num_index) {
        case INT_SIZE_8: imm->v_uint8 = im1.v_uint8; break;
        case INT_SIZE_16: imm->v_uint16 = im1.v_uint16; break;
        case INT_SIZE_32: imm->v_uint32 = im1.v_uint32; break;
        case INT_SIZE_64: imm->v_uint64 = im1.v_uint64; break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    }
    case NUM_KIND_INT: {
      switch (im1.num_index) {
        case INT_SIZE_8: imm->v_int8 = (int8_t)labs(im1.v_int8); break;
        case INT_SIZE_16: imm->v_int16 = (int16_t)labs(im1.v_int16); break;
        case INT_SIZE_32: imm->v_int32 = (int32_t)labs(im1.v_int32); break;
        case INT_SIZE_64: imm->v_int64 = (int64_t)labs(im1.v_int64); break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    }
    ...
    case NUM_KIND_IMAG:
      switch (im1.num_index) {
        case FLOAT_SIZE_32: imm->v_float32 = fabsf(im1.v_float32); break;
        case FLOAT_SIZE_64: imm->v_float64 = fabs(im1.v_float64); break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
    case NUM_KIND_COMPLEX:
...
static void doFoldGetReal(chpl::Context* context,
  ...
  switch (im1.const_kind) {
    case NUM_KIND_COMPLEX:
      switch (im1.num_index) {
        case COMPLEX_SIZE_64: imm->v_float32 = im1.v_complex64.r; break;
        case COMPLEX_SIZE_128: imm->v_float64 = im1.v_complex128.r; break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
  }
...
static void doFoldGetImag(chpl::Context* context,
  ...
  switch (im1.const_kind) {
    case NUM_KIND_COMPLEX:
      switch (im1.num_index) {
        case COMPLEX_SIZE_64: imm->v_float32 = im1.v_complex64.i; break;
        case COMPLEX_SIZE_128: imm->v_float64 = im1.v_complex128.i; break;
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");
      }
      break;
  }
...
#define DO_FOLDB(_op, _complex_combine)                                       \
  switch (im1.const_kind) {                                                   \
    ...
    case NUM_KIND_UINT: {                                                     \
      switch (im1.num_index) {                                                \
        case INT_SIZE_8: imm->v_bool = im1.v_uint8 _op im2.v_uint8; break;    \
        case INT_SIZE_16: imm->v_bool = im1.v_uint16 _op im2.v_uint16; break; \
        case INT_SIZE_32: imm->v_bool = im1.v_uint32 _op im2.v_uint32; break; \
        case INT_SIZE_64: imm->v_bool = im1.v_uint64 _op im2.v_uint64; break; \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");  \
      }                                                                       \
      break;                                                                  \
    }                                                                         \
    case NUM_KIND_INT: {                                                      \
      switch (im1.num_index) {                                                \
        case INT_SIZE_8: imm->v_bool = im1.v_int8 _op im2.v_int8; break;      \
        case INT_SIZE_16: imm->v_bool = im1.v_int16 _op im2.v_int16; break;   \
        case INT_SIZE_32: imm->v_bool = im1.v_int32 _op im2.v_int32; break;   \
        case INT_SIZE_64: imm->v_bool = im1.v_int64 _op im2.v_int64; break;   \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement");  \
      }                                                                       \
      break;                                                                  \
    }                                                                         \
    ...
    case NUM_KIND_COMPLEX:                                                    \
      switch (im1.num_index) {                                                \
        case COMPLEX_SIZE_64:                                                 \
          imm->v_bool =                                                       \
            (im1.v_complex64.r _op im2.v_complex64.r)_complex_combine(        \
              im1.v_complex64.i _op im2.v_complex64.i);                       \
          break;                                                              \
        case COMPLEX_SIZE_128:                                                \
          imm->v_bool =                                                       \
            (im1.v_complex128.r _op im2.v_complex128.r)_complex_combine(      \
  ...
  switch (imm->const_kind) {                                                 \
    ...
    case NUM_KIND_INT: {                                                     \
      switch (imm->num_index) {                                              \
        ...
        case INT_SIZE_16: imm->v_int16 = im1.v_int16 _op im2.v_int16; break; \
        case INT_SIZE_32: imm->v_int32 = im1.v_int32 _op im2.v_int32; break; \
        case INT_SIZE_64: imm->v_int64 = im1.v_int64 _op im2.v_int64; break; \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement"); \
      }                                                                      \
      break;                                                                 \
    }                                                                        \
...
#define DO_FOLD1(_op)                                                        \
  switch (imm->const_kind) {                                                 \
    case NUM_KIND_NONE: break;                                               \
    case NUM_KIND_BOOL: {                                                    \
      imm->v_bool = _op im1.bool_value();                                    \
      break;                                                                 \
    }                                                                        \
    case NUM_KIND_UINT: {                                                    \
      switch (imm->num_index) {                                              \
        case INT_SIZE_8: imm->v_uint8 = _op im1.v_uint8; break;              \
        case INT_SIZE_16: imm->v_uint16 = _op im1.v_uint16; break;           \
        case INT_SIZE_32: imm->v_uint32 = _op im1.v_uint32; break;           \
        case INT_SIZE_64: imm->v_uint64 = _op im1.v_uint64; break;           \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement"); \
      }                                                                      \
      break;                                                                 \
    }                                                                        \
    case NUM_KIND_INT: {                                                     \
      switch (imm->num_index) {                                              \
        case INT_SIZE_8: imm->v_int8 = _op im1.v_int8; break;                \
        case INT_SIZE_16: imm->v_int16 = _op im1.v_int16; break;             \
        case INT_SIZE_32: imm->v_int32 = _op im1.v_int32; break;             \
        case INT_SIZE_64: imm->v_int64 = _op im1.v_int64; break;             \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement"); \
      }                                                                      \
      break;                                                                 \
    }                                                                        \
    ...
    case NUM_KIND_IMAG:                                                      \
      switch (imm->num_index) {                                              \
        case FLOAT_SIZE_32: imm->v_float32 = _op im1.v_float32; break;       \
        case FLOAT_SIZE_64: imm->v_float64 = _op im1.v_float64; break;       \
        default: CHPL_ASSERT(false && "Unhandled case in switch statement"); \
      }                                                                      \
      break;                                                                 \
    case NUM_KIND_COMPLEX:                                                   \
...
static int num_kind_int_to_float(int num_index) {
...
void fold_result(Immediate* im1, Immediate* im2, Immediate* imm) {
  ...
  if (im1->const_kind == NUM_KIND_COMPLEX ||
    ...
    if (im2->const_kind == NUM_KIND_UINT || im2->const_kind == NUM_KIND_INT) {
  ...
  if (im1->const_kind == NUM_KIND_IMAG || im2->const_kind == NUM_KIND_IMAG) {
  ...
  if (im1->const_kind == NUM_KIND_REAL || im2->const_kind == NUM_KIND_REAL) {
...
void fold_constant(chpl::Context* context,
                   int op,
                   Immediate* aim1,
                   Immediate* aim2,
                   Immediate* imm) {
...
ImmString istrFromUserComplex(chpl::Context* context, double re, double im) {
  char s[140];
