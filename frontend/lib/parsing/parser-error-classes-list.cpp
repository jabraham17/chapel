...
void ErrorBisonSyntaxError::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    ...
    type_,
    loc,
    (nearestToken.empty() ? "(no token given)" : "near '" + nearestToken + "'"),
    ":");
  wr.code(loc);
}
...
void ErrorBisonUnknownError::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, loc, "unknown error from Bison parser: ", errorMessage, ".");
  if (!nearestToken.empty()) {
    wr.note(
      loc, "Nearest token when error was encountered: '", nearestToken, "'");
  }
}
...
void ErrorLabelIneligibleStmt::write(ErrorWriterBase& wr) const {
  ...
  if (maybeStmt && maybeStmt->tag() == uast::AstTag::EmptyStmt) {
    ...
    wr.heading(
      kind_, type_, loc, "labels should not be terminated with semicolons.");
  } else {
...
void ErrorNonAssociativeComparison::write(ErrorWriterBase& wr) const {
  ...
  bool allSimple =
    std::all_of(operands.begin(), operands.end(), [](auto operand) {
  ...
  wr.heading(
    kind_, type_, ops.front(), "comparison operators are not associative.");
  ...
  if (allSimple && (types == 0b1 || types == 0b10)) {
    ...
    wr.message(
      "If you wanted to perform elementwise comparison, consider using "
  ...
  wr.message(
    "If you wanted to use the result of a comparison as an operand in "
...
void ErrorNewWithoutArgs::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, loc, "'new' expression is missing its argument list.");
  wr.message("'new' expression used here:");
  wr.code(loc, {expr});
...
void ErrorStringLiteralEOF::write(ErrorWriterBase& wr) const {
  ...
  wr.code(loc);
  wr.message("This string literal began with ",
             startDelimiter,
...
void ErrorEmptyEnum::write(ErrorWriterBase& wr) const {
  auto loc = std::get<const Location>(info_);
  wr.heading(kind_, type_, loc, "enums cannot be empty.");
...
void ErrorCantApplyPrivate::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, node, "can't apply private to ", appliedToWhat, " yet.");
...
void ErrorWhenAfterOtherwise::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, node, "'otherwise' clause must follow all 'when' clauses.");
  ...
  wr.code(when);
  wr.message("Chapel requires 'otherwise' clauses to occur last within "
             "'select' statements.");
}
...
void ErrorDisallowedControlFlow::write(ErrorWriterBase& wr) const {
  ...
  const uast::Function* blockingAstFn =
    blockingAst ? blockingAst->toFunction() : nullptr;
  bool isFuncWithReturn = ret && blockingAstFn;
  bool isIterWithReturn =
    isFuncWithReturn && blockingAstFn->kind() == uast::Function::ITER;
  if (isIterWithReturn) {
    wr.heading(kind_,
               type_,
               ret,
    ...
    wr.note(locationOnly(blockingAstFn),
            "'",
            blockingAstFn->name(),
    ...
    if (allowingAst != nullptr) {
      ...
      // out.
      wr.note(locationOnly(allowingFn),
              "'",
              blockingAstFn->name(),
              "' is declared inside '",
              allowingFn->name(),
      ...
      wr.message(
        "Did you mean to use the 'yield' keyword instead of 'return'?");
    }
    return;
    ...
    wr.heading(kind_,
               type_,
               ret,
               "'return' statements with values are not allowed in ",
               blockingAstFn->name());
    wr.message("The following 'return' statement has a value:");
    wr.code(ret, {ret->value()});
    wr.codeForLocation(blockingAstFn);
  ...
  if (!labelName.empty()) {
    ...
    if (!allowingAst) {
      ...
      wr.heading(kind_,
                 ...
                 " is not allowed.");
      wr.code(invalidAst, {invalidAst});
      wr.note(locationOnly(blockingAst),
              "cannot '",
              astType,
              "' "
              "out of ",
...
void ErrorInvalidGpuAttribute::write(ErrorWriterBase& wr) const {
  ...
  if (loopTypes) {
    wr.heading(kind,
               ...
               ", but '",
               loopTypes,
               "' loops don't support GPU execution.");
    whatIsAffected = "loop";
  } else if (node->isFunction()) {
    wr.heading(kind,
               type,
               node,
               "functions do not currently support the @",
               attr->name(),
               " attribute.");
...
void ErrorInvalidParenfulDeprecation::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             attributeGroup,
             "the '@deprecated' attribute with 'parenful=true' can only be "
             "applied to parenless functions.");
  wr.message("It is used to indicate that a parenless function used to be "
             "callable with parentheses.");
...
void ErrorUnsupportedAsIdent::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, locationOnly(as), "this form of 'as' is not yet supported.");
...
void ErrorAmbiguousMain::write(ErrorWriterBase& wr) const {
  ...
  if (allSameModule) {
    ...
    if (modNames.size() > 0) moduleName = modNames[0].c_str();

    wr.heading(kind_,
               type_,
               loc,
               "multiple 'main' procedures in module '",
               moduleName,
               "'");
    wr.message("");
    for (auto fn : mainFns) {
    ...
    for (auto fn : mainFns) {
      const char* moduleName = "<unknown>";
      if (i < modNames.size()) moduleName = modNames[i].c_str();
      wr.note(
        locationOnly(fn), "found 'proc main' in module '", moduleName, "'");
      wr.codeForDef(fn);
      i++;
    }
  }
...
void ErrorAmbiguousMainModule::write(ErrorWriterBase& wr) const {
  ...
  for (auto mod : modules) {
    wr.note(
      locationOnly(mod), "'module ", mod->name(), "' could be the main module");
    wr.codeForDef(mod);
  }
}
