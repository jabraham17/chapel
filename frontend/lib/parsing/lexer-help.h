...
static YYLTYPE
getLocation(yyscan_t scanner, int nLines, int nCols, bool moveToEnd = false) {
...
int processNewline(yyscan_t scanner) {
  ...
  if (context->parseStats) context->parseStats->countNewline();
  return YYLEX_NEWLINE;
}

static void
syntax(yyscan_t scanner, int nLines, int nCols, const char* fmt, ...) {
...
static int processIdentifier(yyscan_t scanner, char specialInitialChar) {
  ...
  if (specialInitialChar == '?')
    tokenType = TQUERIEDIDENT;
  else if (specialInitialChar == '@')
    tokenType = TATTRIBUTEIDENT;
...
static int processToken(yyscan_t scanner, int t) {
  ...
  ParserContext* context = yyget_extra(scanner);
  if (context->parseStats) context->parseStats->countToken(pch);
  YYLTYPE* loc = yyget_lloc(scanner);
...
static int
processStringLiteral(yyscan_t scanner, const char* startChar, int type) {
  ...
  std::string value =
    eatStringLiteral(scanner, startChar, nColOffset, erroneous);
  ...
  if (startChar && startChar[0] == '"') quotes = StringLiteral::DOUBLE;
...
static int
processTripleStringLiteral(yyscan_t scanner, const char* startChar, int type) {
  // figure out the location offset for the string start (e.g. b" )
  const char* pch = yyget_text(scanner);
  int nColOffset = (int)strlen(pch);
  YYLTYPE* loc = yyget_lloc(scanner);
  ...
  bool erroneous = false;
  std::string value =
    eatTripleStringLiteral(scanner, startChar, nColOffset, erroneous);
  ...
  if (startChar && startChar[0] == '"') quotes = StringLiteral::TRIPLE_DOUBLE;
...
static char simpleEscape(int c) {
  if (c == '\'') return '\'';
  if (c == '"') return '"';
  if (c == '?') return '?';
  if (c == '\\') return '\\';
  ...
  while (c != startCh && c != 0) {
    if (c == '\n') {
      ...
      if (esc != 0) {
        ...
        CHPL_ASSERT(
          (hexChar != LONG_MIN && hexChar != LONG_MAX) &&
          "overflow/underflow shouldn't be possible in the allowed number "
          "of digits");
        if (!foundHex) {
          syntax(
            scanner, nLines, nCols, "non-hexadecimal character follows \\x.");
          isErroneousOut = true;
        ...
        if (foundNonHex) continue; // need to process c as the next character
        ...
        syntax(
          scanner, nLines, nCols, "unexpected string escape: '\\%c'.", (char)c);
        isErroneousOut = true;
      }
    } else {
      s += c;
...
static std::string eatTripleStringLiteral(yyscan_t scanner,
  ...
  if (c == 0) {
    CHPL_LEXER_REPORT(
      scanner, nLines, nCols, StringLiteralEOF, startCh, startChCount);
    isErroneousOut = true;
  }
...
static SizedStr eatExternCode(yyscan_t scanner) {
  ...
  while (depth > 0) {
    ...
    switch (state) {
      ...
      case in_single_quote_backslash: state = in_single_quote; break;
      ...
      case in_double_quote_backslash: state = in_double_quote; break;
...
static int processSingleLineComment(yyscan_t scanner) {
  ...
  ParserContext* context = yyget_extra(scanner);
  if (context->parseStats) context->parseStats->countCommentLine();
...
static int processBlockComment(yyscan_t scanner) {
  ...
  while (depth > 0) {
    ...
    if (lastc == '*' && c == '/' && lastlastc != '/') { // close comment
      ...
      CHPL_LEXER_REPORT_END(
        scanner, nLines, nCols, CommentEOF, startLoc, nestedLoc);
      break;
    }
  }
