...
static const LookupConfig IN_SCOPE_CONFIG =
  LOOKUP_DECLS | LOOKUP_PARENTS | LOOKUP_METHODS;

static MatchingIdsWithName getMatchingIdsInInternalModule(Context* context,
                                                          const char* modName,
                                                          UniqueString name) {
  auto mod =
    parsing::getToplevelModule(context, UniqueString::get(context, modName));

  if (!mod) return MatchingIdsWithName();

  return lookupNameInScope(context,
                           scopeForModule(context, mod->id()),
...
static bool areFnOverloadsPresentInDefiningScope(Context* context,
                                                 const Type* type,
  ...
  for (const auto& id : ids) {
    ...
    if (auto fn = node->toFunction()) {
      if (fn->isMethod() || fn->kind() == Function::Kind::OPERATOR) {
        ...
        auto checkFormal = (fn->isMethod() ? fn->thisFormal() : fn->formal(0));
...
template <typename F>
const TypedFnSignature* typedSignatureFromGenerator(ResolutionContext* rc,
                                                    F&& generator,
                                                    const ID& generateFor) {
...
generateInitCopySignature(ResolutionContext* rc,
                          const CompositeType* inCompType) {
  return typedSignatureFromGenerator(rc, buildInitEquals, inCompType->id());
}

static const TypedFnSignature*
generateRecordComparisonSignature(ResolutionContext* rc,
...
generateRecordInequalityComparisonSignature(ResolutionContext* rc,
                                            const CompositeType* lhsType) {
  return typedSignatureFromGenerator(
    rc, buildRecordInequalityComparison, lhsType->id());
}

static const TypedFnSignature*
generateRecordCompareLtSignature(ResolutionContext* rc,
                                 const CompositeType* lhsType) {
  return typedSignatureFromGenerator(rc, buildRecordCompareLt, lhsType->id());
}

static const TypedFnSignature*
generateRecordCompareLeSignature(ResolutionContext* rc,
                                 const CompositeType* lhsType) {
  return typedSignatureFromGenerator(rc, buildRecordCompareLe, lhsType->id());
}

static const TypedFnSignature*
generateRecordCompareGtSignature(ResolutionContext* rc,
                                 const CompositeType* lhsType) {
  return typedSignatureFromGenerator(rc, buildRecordCompareGt, lhsType->id());
}

static const TypedFnSignature*
generateRecordCompareGeSignature(ResolutionContext* rc,
                                 const CompositeType* lhsType) {
...
static const TypedFnSignature*
generateRecordHashSignature(ResolutionContext* rc,
                            const CompositeType* lhsType) {
  return typedSignatureFromGenerator(rc, buildRecordHash, lhsType->id());
}

using EnumCastSelector = OverloadSelector<buildEnumToStringCastImpl,
                                          buildEnumToBytesCastImpl,
                                          buildStringToEnumCastImpl,
                                          buildBytesToEnumCastImpl>;

using CompositeGeneratorType =
  TypedFnSignature const* (*)(ResolutionContext*, const CompositeType*);
using EnumGeneratorType = TypedFnSignature const* (*)(ResolutionContext*,
                                                      const EnumType*);

static CompositeGeneratorType
generatorForCompilerGeneratedRecordOperator(UniqueString name) {
  if (name == USTR("==")) {
...
static void setupGeneratedEnumCastFormals(
  Context* context,
...
generateIntegralToOrFromCastForEnum(ResolutionContext* rc,
  ...
  setupGeneratedEnumCastFormals(
    context, enumType, ufsFormals, formalTypes, isFromCast);
...
static EnumGeneratorType
generatorForCompilerGeneratedEnumOperator(UniqueString name,
                                          const QualifiedType& lhs,
                                          const QualifiedType& rhs,
                                          const EnumType*& outEt) {
  auto lhsT = lhs.type();
  auto rhsT = rhs.type();
  ...
  if (name == USTR(":")) {
    if ((outEt = lhsT->toEnumType())) {
      ...
      if (rhsT->isIntegralType())
        return generateIntegralToOrFromCastForEnum</* isFromCast */ true>;
    } else if ((outEt = rhsT->toEnumType())) {
      if (lhsT->isIntegralType())
        return generateIntegralToOrFromCastForEnum</* isFromCast */ false>;
...
static bool isCompilerGeneratedEnumOperator(UniqueString name,
                                            const QualifiedType& lhs,
                                            const QualifiedType& rhs,
                                            QualifiedType& outTypeForScope) {
  const EnumType* et = nullptr;
  if (generatorForCompilerGeneratedEnumOperator(name, lhs, rhs, et) !=
      nullptr) {
...
bool needCompilerGeneratedMethod(Context* context,
  ...
  if ((isAggregate && isNameOfCompilerGeneratedMethod(name))) {
    bool userDefinedExists = areFnOverloadsPresentInDefiningScope(
      context, type, QualifiedType::INIT_RECEIVER, name);
    ...
    if (name == USTR("hash")) {
      ...
      userDefinedExists |=
        areOperatorOverloadsPresentInDefiningScope(
          context, /* typeForScope */ qt, qt, qt, USTR("==")) ||
        areOperatorOverloadsPresentInDefiningScope(
          context, /* typeForScope */ qt, qt, qt, USTR("!="));
    }
...
static bool needCompilerGeneratedBinaryOpImpl(Context* context,
  ...
  if (lhsT->isExternType() && name == USTR("=")) {
    ...
    if (!areFnOverloadsPresentInDefiningScope(
          context, lhs.type(), QualifiedType::INIT_RECEIVER, name)) {
      return true;
    }
  }

  auto lhsRec = lhsT->toRecordType(), rhsRec = rhsT->toRecordType();
  if (lhsRec && rhsRec && isNameOfCompilerGeneratedRecordOperator(name)) {
    ...
    if (!areOperatorOverloadsPresentInDefiningScope(
          context, /* typeForScope */ lhs, lhs, rhs, name)) {
      return true;
    }
  }
  ...
  if (isCompilerGeneratedEnumOperator(name, lhs, rhs, enumType)) {
    if (!areOperatorOverloadsPresentInDefiningScope(
          context, /* typeForScope */ enumType, lhs, rhs, name)) {
      return true;
    }
  }
...
bool needCompilerGeneratedBinaryOp(Context* context,
  ...
  for (int i : {0b00, 0b01, 0b10, 0b11}) {
    ...
    if (i & 0b01) {
      ...
      if (!lhsScalar.isUnknownOrErroneous())
        testLhs = lhsScalar;
      else
        continue; // no promotion for lhs, skip this case
    }

    if (i & 0b10) {
      auto rhsScalar = getPromotionType(context, rhs);
      if (!rhsScalar.isUnknownOrErroneous())
        testRhs = rhsScalar;
      else
        continue; // no promotion for rhs, skip this case
    }
...
static bool initHelper(Context* context,
  ...
  if (auto cls = typeDecl->toClass()) {
    ...
    if (pct) {
      ...
      auto parentReceiver = ClassType::get(
        context, pct->toBasicClassType(), manager, borrowedNonnilDecor);
      auto userDefinedExists = areFnOverloadsPresentInDefiningScope(
        context, parentReceiver, QualifiedType::INIT_RECEIVER, USTR("init"));
      ...
      if (!userDefinedExists) {
        ...
        for (auto formal : fn->formals()) {
          if (auto named = formal->toNamedDecl();
            ...
            owned<AstNode> arg =
              Identifier::build(builder, dummyLoc, named->name());
  ...
  auto addFormal = [&](const VarLikeDecl* field,
    ...
    owned<AstNode> formal =
      Formal::build(builder,
                    dummyLoc,
    ...
    owned<AstNode> assign = OpCall::build(
      builder, dummyLoc, USTR("="), std::move(lhs), std::move(rhs));
    stmts.push_back(std::move(assign));

    formals.push_back(std::move(formal));
  };

  std::function<void(const AstNode*)> processDecl = [&addFormal,
                                                     &processDecl](auto ast) {
...
const BuilderResult& buildInitializer(Context* context, ID typeID) {
  ...
  auto thisFormal = Formal::build(builder,
                                  dummyLoc,
                                  nullptr,
                                  USTR("this"),
  ...
  if (addSuperInit) {
    owned<AstNode> dot =
      Dot::build(builder,
                 dummyLoc,
                 Identifier::build(builder, dummyLoc, USTR("super")),
                 USTR("init"));
    owned<AstNode> call = FnCall::build(builder,
                                        dummyLoc,
                                        std::move(dot),
                                        std::move(superArgs),
...
buildInitUfsFormals(const uast::Function* initFn) {
  ...
  for (auto decl : initFn->formals()) {
    ...
    auto fd = UntypedFnSignature::FormalDetail(
      name, defaultKind, decl, decl->isVarArgFormal());
...
generateInitSignature(ResolutionContext* rc, const CompositeType* inCompType) {
  ...
  auto uSig = UntypedFnSignature::get(context,
                                      initFn->id(),
                                      initFn->name(),
                                      true,
                                      /* isTypeConstructor */ false,
...
struct FnBuilder {
  ...
  FnBuilder(Context* context,
    ...
    : context_(context), typeID_(std::move(typeID)), name_(name), kind_(kind),
      throws_(throws) {
    builder_ =
      Builder::createForGeneratedCode(context_, typeID_, overloadOffset);
  ...
  owned<StringLiteral> stringLit(const char* str) {
    return StringLiteral::build(
      builder(), dummyLoc_, str, StringLikeLiteral::QuoteStyle::DOUBLE);
  }

  owned<BytesLiteral> bytesLit(const char* str) {
    return BytesLiteral::build(
      builder(), dummyLoc_, str, StringLikeLiteral::QuoteStyle::DOUBLE);
  }
  ...
  owned<IntLiteral> intLit(int64_t value) {
    return IntLiteral::build(
      ...
      dummyLoc_,
      value,
      UniqueString::get(context_, std::to_string(value)));
  }
  ...
  owned<Identifier> identifier(std::string name) {
    return Identifier::build(
      builder(), dummyLoc_, UniqueString::get(context_, name));
  }
  ...
  owned<OpCall> op(owned<AstNode> lhs, UniqueString op, owned<AstNode> rhs) {
    return OpCall::build(
      builder(), dummyLoc_, op, std::move(lhs), std::move(rhs));
  }
  ...
  owned<AstNode> conditional(owned<AstNode> condition,
    ...
    auto elseBlock =
      elseStmts ? Block::build(builder(), dummyLoc_, std::move(*elseStmts))
                : nullptr;
  ...
  template <typename F> void eachField(F&& callable) {
  ...
  BuilderResult
  finalize(owned<Formal> receiver, AstList otherFormals, owned<Block> body) {
    auto genFn = Function::build(builder(),
                                 dummyLoc_,
                                 {},
                                 Decl::Visibility::PUBLIC,
...
struct BinaryFnBuilder : public FnBuilder {
  ...
  BinaryFnBuilder(Context* context,
                  ID typeID,
                  UniqueString name,
                  Function::Kind kind,
  ...
  owned<Formal>& lhsFormal() {
    ...
    lhsFormal_ = Formal::build(builder(),
                               dummyLoc_,
                               nullptr,
                               nameToUse,
                               lhsIntent_,
  ...
  owned<Formal>& rhsFormal() {
    ...
    rhsFormal_ = Formal::build(builder(),
                               dummyLoc_,
                               nullptr,
                               otherName,
                               rhsIntent_,
  ...
  BuilderResult finalize() {
    ...
    if (kind_ == Function::Kind::OPERATOR) {
      ...
      lhsFormal_ = Formal::build(builder(),
                                 ...
                                 USTR("this"),
                                 Formal::DEFAULT_INTENT,
                                 identifier(typeID_.symbolName(context_)),
                                 nullptr);
    }
    otherFormals.push_back(std::move(rhsFormal()));
    return FnBuilder::finalize(
      std::move(lhsFormal()), std::move(otherFormals), std::move(body));
  }
};

struct FieldFnBuilder : BinaryFnBuilder {
  FieldFnBuilder(Context* context,
                 ID typeID,
                 UniqueString name,
                 Function::Kind kind,
                 Formal::Intent lhsIntent = Formal::DEFAULT_INTENT,
...
const BuilderResult& buildRecordComparison(Context* context, ID typeID) {
  ...
  bld.eachField([&bld](const Variable* decl) {
    ...
    auto neqCall = bld.call(UniqueString::get(bld.context(), "chpl_field_neq"),
                            bld.lhsField(decl),
                            bld.rhsField(decl));
    bld.stmts().push_back(
      bld.earlyReturn(std::move(neqCall), bld.boolLit(false)));
  });
...
const BuilderResult& buildRecordInequalityComparison(Context* context,
                                                     ID typeID) {
...
buildOrderedComparison(Context* context,
                       ID typeID,
                       UniqueString fnName,
  ...
  bld.eachField([&, strictCompFn](const Variable* decl) {
    ...
    bld.stmts().push_back(bld.earlyReturn(
      bld.call(strictCompFn, bld.lhsField(decl), bld.rhsField(decl)),
      bld.boolLit(true)));
    bld.stmts().push_back(bld.earlyReturn(
      bld.call(strictCompReversedFn, bld.lhsField(decl), bld.rhsField(decl)),
      bld.boolLit(false)));
  });
...
const uast::BuilderResult& buildRecordCompareLt(Context* context, ID typeID) {
  ...
  auto result =
    buildOrderedComparison(context,
                           typeID,
                           USTR("<"),
...
const BuilderResult& buildRecordAssign(Context* context, ID typeID) {
  ...
  bld.eachField([&bld](const Variable* decl) {
    if (decl->kind() == Variable::TYPE || decl->kind() == Variable::PARAM)
      return;
...
struct HashFnBuilder : public FnBuilder {
  ...
  HashFnBuilder(Context* context, ID typeID)
    : FnBuilder(
        context, typeID, USTR("hash"), Function::PROC, false, chpl::empty) {
    thisFormal_ = Formal::build(builder(),
                                dummyLoc_,
                                nullptr,
                                USTR("this"),
                                Formal::DEFAULT_INTENT,
  ...
  void accumulateHash() {
    ...
    eachField([&hashExpr, &fieldIdx, this](const Variable* decl) {
      ...
      auto fieldAccess =
        this->dot(this->identifier(USTR("this")), decl->name());
      if (!hashExpr) {
        hashExpr = this->call(
          UniqueString::get(this->context(), "chpl__defaultHashWrapperInner"),
          std::move(fieldAccess));
      } else {
        auto hashCall =
          this->call(this->dot(std::move(fieldAccess), USTR("hash")));
        hashExpr = this->call(
          UniqueString::get(this->context(), "chpl__defaultHashCombine"),
          std::move(hashCall),
          std::move(hashExpr),
    ...
    if (!hashExpr) {
      ...
      hashExpr =
        this->op(this->intLit(0), USTR(":"), this->identifier(USTR("uint")));
      isInline_ = true;
    }
...
static const TypedFnSignature*
generateDeinitSignature(ResolutionContext* rc,
                        const CompositeType* inCompType) {
  auto context = rc->context();
  std::vector<UntypedFnSignature::FormalDetail> ufsFormals;
...
buildDeSerialize(Context* context, ID typeID, bool isSerializer) {
  ...
  auto thisType =
    Identifier::build(builder, dummyLoc, typeID.symbolName(context));
  auto thisFormal = Formal::build(builder,
                                  dummyLoc,
                                  nullptr,
                                  USTR("this"),
  ...
  auto writerArg = Formal::build(builder,
                                 dummyLoc,
                                 nullptr,
                                 UniqueString::get(context, "writer"),
                                 Formal::DEFAULT_INTENT,
                                 {},
                                 nullptr);
  auto serializerArg = Formal::build(builder,
                                     dummyLoc,
                                     nullptr,
                                     UniqueString::get(context, "serializer"),
                                     Formal::REF,
                                     {},
                                     nullptr);
  AstList formals;
  ...
  AstList stmts;
  FnBuilder helper(
    context, typeID, UniqueString(), Function::Kind::PROC, false);
  auto call =
    helper.call(helper.identifier("serializeDefaultImpl"),
...
static void buildWhenStmts(Context* context,
                           Builder* builder,
                           Location dummyLoc,
                           const Enum* type,
                           AstList& whenStmts,
  ...
  for (auto elem : type->enumElements()) {
    owned<AstNode> elemIdent =
      Identifier::build(builder, dummyLoc, elem->name());
    ...
    AstList stmtList;
    auto ret = Return::build(
      builder, dummyLoc, std::move(enumToOrder ? intLit : elemIdent));
    stmtList.push_back(std::move(ret));

    auto when = When::build(builder,
                            dummyLoc,
                            std::move(cases),
                            BlockStyle::IMPLICIT,
                            std::move(stmtList));
...

  {
    ...
    auto prim =
      PrimCall::build(builder, dummyLoc, uast::primtags::PRIM_RT_ERROR, {});

    AstList stmtList;
    stmtList.push_back(std::move(prim));

    auto when = When::build(
      builder, dummyLoc, {}, BlockStyle::IMPLICIT, std::move(stmtList));
    whenStmts.push_back(std::move(when));
  }
}
...
static owned<Use>
useDefsFromChapelBase(Context* context,
                      Builder* builder,
                      const Location& dummyLoc,
                      std::vector<const char*> limitationStrs = {"=="}) {
  AstList baseUseList;
  auto baseIdent = Identifier::build(
    builder, dummyLoc, UniqueString::get(context, "ChapelBase"));
  std::vector<owned<AstNode>> limitations;
  for (auto limitation : limitationStrs) {
    auto eq = Identifier::build(
      builder, dummyLoc, UniqueString::get(context, limitation));
    limitations.push_back(std::move(eq));
  }
  auto baseVisClause = VisibilityClause::build(builder,
                                               dummyLoc,
                                               std::move(baseIdent),
  ...
  auto useBase = Use::build(builder,
                            dummyLoc,
                            Decl::Visibility::DEFAULT_VISIBILITY,
                            std::move(baseUseList));
  return useBase;
}
...
const BuilderResult& buildEnumToOrder(Context* context, ID typeID) {
  ...
  auto argFormal = Formal::build(builder,
                                 dummyLoc,
                                 nullptr,
                                 argName,
                                 Formal::DEFAULT_INTENT,
...

  {
    AstList enumUseList;
    auto visClause =
      VisibilityClause::build(builder, dummyLoc, std::move(typeIdent));
    enumUseList.push_back(std::move(visClause));
    auto useEnum = Use::build(builder,
                              dummyLoc,
                              Decl::Visibility::DEFAULT_VISIBILITY,
                              std::move(enumUseList));
  ...
  AstList whenStmts;
  buildWhenStmts(
    context, builder, dummyLoc, type, whenStmts, /*enumToOrder=*/true);

  auto expr = Identifier::build(builder, dummyLoc, argName);
  auto select =
    Select::build(builder, dummyLoc, std::move(expr), std::move(whenStmts));
...
struct EnumCastBuilder : BinaryFnBuilder {
  ...
  EnumCastBuilder(Context* context,
    ...
    : BinaryFnBuilder(context,
      ...
      otherType_(otherType), castFromEnum_(castFromEnum) {}
  ...
  void addWhenClause(owned<AstNode> whenValue, owned<AstNode> toReturn) {
    AstList caseExprs;
    caseExprs.push_back(std::move(whenValue));
    AstList stmts;
    stmts.push_back(ret(std::move(toReturn)));
    auto whenClause = When::build(builder(),
                                  dummyLoc_,
                                  std::move(caseExprs),
                                  BlockStyle::IMPLICIT,
                                  std::move(stmts));
  ...
  void setFallback(owned<AstNode> fallback) { fallback_ = std::move(fallback); }

  BuilderResult finalize() {
    stmts().push_back(useDefsFromChapelBase(
      context(), builder(), dummyLoc_, {"==", "chpl_enum_cast_error"}));
    auto select = Select::build(builder(),
                                dummyLoc_,
    ...
    if (fallback_) {
      ...
      stmts().push_back(
        call(UniqueString::get(context(), "chpl_enum_cast_error"),
             identifier(lhsFormal()->name()),
...
buildEnumToStringOrBytesCastImpl(Context* context,
                                 ID typeID,
                                 bool isString,
                                 bool castFromEnum,
                                 int overloadIdx) {
  QUERY_BEGIN(buildEnumToStringOrBytesCastImpl,
              context,
              typeID,
              isString,
              castFromEnum,
  ...
  EnumCastBuilder builder(context,
                          typeID,
                          isString ? USTR("string") : USTR("bytes"),
                          castFromEnum,
                          overloadIdx);
  ...
  for (auto elt : enumDecl->enumElements()) {
    if (castFromEnum) {
      ...
      builder.addWhenClause(
        buildLiteral(UniqueString::getConcat(
          context, enumDecl->name().c_str(), ".", elt->name().c_str())),
        builder.enumElt(elt->name()));
    }
  }
...
buildEnumToStringCastImpl(Context* context, ID typeID, int overloadIdx) {
  return buildEnumToStringOrBytesCastImpl(
    context, typeID, true, /* castFromEnum */ true, overloadIdx);
}
const uast::BuilderResult&
buildEnumToBytesCastImpl(Context* context, ID typeID, int overloadIdx) {
  return buildEnumToStringOrBytesCastImpl(
    context, typeID, false, /* castFromEnum */ true, overloadIdx);
}

const uast::BuilderResult&
buildStringToEnumCastImpl(Context* context, ID typeID, int overloadIdx) {
  return buildEnumToStringOrBytesCastImpl(
    context, typeID, true, /* castFromEnum */ false, overloadIdx);
}
const uast::BuilderResult&
buildBytesToEnumCastImpl(Context* context, ID typeID, int overloadIdx) {
  return buildEnumToStringOrBytesCastImpl(
    context, typeID, false, /* castFromEnum */ false, overloadIdx);
}
...
const BuilderResult& buildOrderToEnum(Context* context, ID typeID) {
  ...
  auto intFormal = Formal::build(builder,
                                 dummyLoc,
                                 nullptr,
                                 intName,
                                 Formal::DEFAULT_INTENT,
  ...
  AstList whenStmts;
  buildWhenStmts(
    context, builder, dummyLoc, type, whenStmts, /*enumToOrder=*/false);
...
static const TypedFnSignature*
generateDeSerialize(ResolutionContext* rc,
  ...
  auto& br =
    buildDeSerialize(context, compType->id(), name == USTR("serialize"));
  auto genFn = br.topLevelExpression(0)->toFunction();

  ufsFormals.push_back(UntypedFnSignature::FormalDetail(
    USTR("this"), UntypedFnSignature::DK_NO_DEFAULT, genFn->thisFormal()));
...
static const TypedFnSignature*
generateTupleMethod(Context* context, const TupleType* at, UniqueString name) {
  ...
  formals.push_back(UntypedFnSignature::FormalDetail(
    USTR("this"), UntypedFnSignature::DK_NO_DEFAULT, nullptr));
...
fieldAccessorQuery(Context* context,
  ...
  auto ufsReceiver = UntypedFnSignature::FormalDetail(
    USTR("this"), UntypedFnSignature::DK_NO_DEFAULT, nullptr);
...
static void generateOperatorFormalDetail(
...
static const TypedFnSignature* generatePtrMethod(Context* context,
  ...
  auto qual =
    receiverType.isType() ? QualifiedType::TYPE : QualifiedType::CONST_REF;
...
static const TypedFnSignature* generateEnumMethod(ResolutionContext* rc,
                                                  const EnumType* et,
                                                  UniqueString name) {
  const TypedFnSignature* result = nullptr;
  auto context = rc->context();
  if (name == USTR("size")) {
    if (!areFnOverloadsPresentInDefiningScope(
          context, et, QualifiedType::TYPE, name)) {
      // TODO: we should really have a way to just set the return type here
      std::vector<UntypedFnSignature::FormalDetail> formals;
      std::vector<QualifiedType> formalTypes;
...
static const TypedFnSignature* generateIteratorMethod(Context* context,
                                                      const IteratorType* it,
                                                      UniqueString name) {

  const TypedFnSignature* result = nullptr;
  if (name == "_shape_" && shapeForIterator(context, it)) {
    std::vector<UntypedFnSignature::FormalDetail> formals;
    std::vector<QualifiedType> formalTypes;
...
static const TypedFnSignature* const&
getCompilerGeneratedMethodQuery(ResolutionContext* rc,
                                QualifiedType receiverType,
                                UniqueString name,
                                bool parenless) {
  CHPL_RESOLUTION_QUERY_BEGIN(
    getCompilerGeneratedMethodQuery, rc, receiverType, name, parenless);
  auto context = rc->context();
  ...
  if (needCompilerGeneratedMethod(context, type, name, parenless)) {
    ...
    if (name == USTR("init")) {
      ...
      result =
        generateDeSerialize(rc, compType, name, "reader", "deserializer");
    } else if (name == USTR("hash")) {
...
getCompilerGeneratedBinaryOp(ResolutionContext* rc,
                             ...
                             UniqueString name) {
  CHPL_RESOLUTION_QUERY_BEGIN(
    getCompilerGeneratedBinaryOp, rc, lhsType, rhsType, name);
  auto context = rc->context();
  ...
  for (int i : {0b00, 0b01, 0b10, 0b11}) {
    ...
    if (needCompilerGeneratedBinaryOpImpl(context, testLhs, testRhs, name)) {
      if (const EnumType* enumType = nullptr;
          auto generator = generatorForCompilerGeneratedEnumOperator(
            name, testLhs, testRhs, enumType)) {
        tfs = generator(rc, enumType);
      } else if (auto recordType = lhsT->toRecordType()) {
        if (auto generator =
              generatorForCompilerGeneratedRecordOperator(name)) {
...
static const TypedFnSignature* const& getParamOrderToEnum(Context* context,
                                                          const EnumType* et) {
  ...
  ufsFormals.push_back(
    UntypedFnSignature::FormalDetail(UniqueString::get(context, "i"),
                                     ...
                                     nullptr));
  formalTypes.push_back(
    QualifiedType(QualifiedType::PARAM, AnyIntegralType::get(context)));
...
static const TypedFnSignature* const& getOrderToEnum(ResolutionContext* rc,
                                                     const EnumType* et) {
  auto context = rc->context();
  std::vector<UntypedFnSignature::FormalDetail> ufsFormals;
...
static const TypedFnSignature* const& getParamEnumToOrder(Context* context,
                                                          const EnumType* et) {
  ...
  ufsFormals.push_back(
    UntypedFnSignature::FormalDetail(UniqueString::get(context, "e"),
                                     UntypedFnSignature::DK_NO_DEFAULT,
                                     nullptr));
  formalTypes.push_back(QualifiedType(QualifiedType::PARAM, et));
...
static const TypedFnSignature* const& getEnumToOrder(ResolutionContext* rc,
                                                     const EnumType* et) {
  auto context = rc->context();
  std::vector<UntypedFnSignature::FormalDetail> ufsFormals;
...
const TypedFnSignature* getCompilerGeneratedFunction(ResolutionContext* rc,
                                                     const CallInfo& ci) {
...
static owned<Function> typeConstructorFnForComposite(Context* context,
  ...
  for (int i = 0; i < nFields; i++) {
    ...
    if (isFieldSyntacticallyGeneric(context, declId, nullptr)) {
      ...
      auto kind =
        fieldDecl->kind() == Variable::PARAM ? Variable::PARAM : Variable::TYPE;
      owned<AstNode> formal =
        Formal::build(builder,
                      fnLoc,
                      /*attributeGroup=*/nullptr,
...
const BuilderResult& buildTypeConstructor(Context* context, ID typeID) {
