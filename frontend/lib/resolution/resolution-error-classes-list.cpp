...
static void describeSymbolTrace(ErrorWriterBase& wr,
  ...
  for (int i = start; i < n; i++) {
    ...
    if (elt.automaticModule) {
      ...
      wr.note(locationOnly(elt.visibilityClauseId),
              errbegin,
              " the '",
...
void ErrorAmbiguousCall::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, locationOnly(callName), "ambiguous call to ", callName);
...
void ErrorAmbiguousConfigName::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             locationOnly(variable),
             "ambiguous config name (",
             name,
             ").");
  wr.code(variable);
  ...
  wr.code(otherId);
  wr.note(
    locationOnly(otherId), "(disambiguate using -s<modulename>.", name, "...)");
}
...
static void
describeAmbiguousMatch(ErrorWriterBase& wr,
  ...
  describeSymbolTrace(wr,
                      ident->id(),
                      ident->name(),
                      trace,
                      0,
                      intro,
                      encounteredAutoModule,
                      from,
                      needsIntroText);
  ...
  if (!encounteredAutoModule) {
    if (match.numIds() == 1 || oneOnly) {
      ...
      if (needsIntroText) {
        wr.note(
          locationOnly(firstId), intro, "found '", from, "' defined here:");
      } else {
...
void ErrorAmbiguousVisibilityIdentifier::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_, type_, mentionId, "'", name, "' is ambiguous");
...
void ErrorAsWithUseExcept::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, locationOnly(use), "cannot rename in 'except' list.");
...
void ErrorAssignFieldBeforeInit::write(ErrorWriterBase& wr) const {
  ...
  for (auto& initPair : initializations) {
    ...
    wr.note(id,
            "field '",
            decl->name(),
            "' is initialized before the 'init' call here:");
    wr.code<ID, ID>(id, {id});
  }
}
...
void ErrorConstRefCoercion::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             ast,
             "function call requires coercion of actual ",
             (c.constRefCoercionActual() + 1),
             " for 'const ref' formal '",
             formalName,
             "'.");
  ...
  wr.message(
    "Formals with the 'const ref' intent do not currently support coercions.");
...
void ErrorDeprecatedSyncRead::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             astForErr,
             "implicitly reading from a 'sync' value is deprecated.");
  wr.codeForLocation(astForErr);
  if (dest != astForErr && dest) {
    wr.note(dest,
            "encountered while trying to pass a 'sync' value to this "
            "non-'sync' destination");
    wr.code(dest, {dest});
  }
  if (source) {
    wr.note(source,
            "to silence this warning, please explicitly call '.read\?\?' on "
            "the 'sync' value here:");
    wr.code(source, {source});
  } else {
    wr.note(astForErr,
            "to silence this warning, please explicitly call '.read\?\?'");
  }
  return;
}
...
void ErrorDotTypeOnType::write(ErrorWriterBase& wr) const {
  ...
  if (haveType) {
    wr.heading(
      kind_, type_, dot, "can't apply '.type' to a type ('", dottedType, "').");
  } else {
...
void ErrorEnumAbstract::write(ErrorWriterBase& wr) const {
  ...
  if (direction == "from" && otherType) {
    wr.heading(kind_,
               type_,
               location,
               "cannot cast from abstract enumeration type '",
               enumType->name(),
               "' to '",
               otherType,
               "'.");
  ...
  wr.message("The type is abstract because none of its constants have been "
...
void ErrorEnumInitializerNotParam::write(ErrorWriterBase& wr) const {
  ...
  wr.message(
    "The initialization expression of '", enumElement->name(), "' is ", qt);
}
...
void ErrorEnumInitializerNotInteger::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             enumElement,
             "only integer (signed or unsigned) values can be associated with "
             "enumeration constants");
  wr.codeForLocation(enumElement);
  wr.message("The initialization expression of '",
             enumElement->name(),
             "' produces ",
             qt.param(),
             ", ",
...
void ErrorEnumValueAbstract::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "'");
  wr.code(location);
  wr.message("The constant '", enumElement->name(), "' is declared here:");
  wr.codeForLocation(enumElement);
  wr.message("The constant is abstract because neither it nor any of the "
             "preceding constants in '",
             enumType->name(),
...
void ErrorIfVarNonClassType::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             var,
             "a variable declared in the condition of "
             "an if ",
             ifKindStr,
             " must be a class, "
...
void ErrorIncompatibleIfBranches::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, ifExpr, "branches of if-expression have incompatible types.");
  ...
  if (qt1.kind() == types::QualifiedType::TYPE ||
    ...
    wr.message("the first branch is ",
               decayToValue(qt1),
               ", while the second is ",
               decayToValue(qt2),
               ".");
    ...
    wr.message("the first branch is of type '",
               qt1.type(),
               "'"
               ", while the second is of type '",
               qt2.type(),
               "'.");
  }
}
...
void ErrorIncompatibleRangeBounds::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, range, "bounds of range expression have incompatible types.");
  ...
  if (qt1.kind() == types::QualifiedType::TYPE ||
    ...
    wr.message("the lower bound is of type '",
               qt1.type(),
               "'"
               ", while the upper bound is of type '",
               qt2.type(),
               "'.");
  }
}
...
void ErrorIncompatibleTypeAndInit::write(ErrorWriterBase& wr) const {
  ...
  if (auto namedDecl = decl->toNamedDecl()) {
    wr.heading(kind_,
               type_,
               decl,
               "type mismatch between declared type of '",
               namedDecl->name(),
  ...
  wr.message("the type specifier has type '",
             typeExprType,
             "', while the "
             "initial value has type '",
             initExprType,
             "'.");
}
...
void ErrorIncompatibleYieldTypes::write(ErrorWriterBase& wr) const {
  ...
  wr.message("All versions of the iterator except the leader should yield the "
             "same type.");
...
static void printRejectedCandidates(
  ErrorWriterBase& wr,
  const ID& anchorId,
  ...
  bool noActuals = (ci.numActuals() == 0 && !ci.isMethodCall()) ||
  ...
  for (auto& candidate : rejected) {
    ...
    if (idForErr.isEmpty() ||
      ...
      candidateDescription =
        std::string("the type constructor for '") + ci.name().c_str() + "'";
    }
    ...
    if (/* skip printing detailed info_ here because computing the formal-actual
      ...
      if (badSplitInit) {
        wr.note(fn->id(),
                candidateDescription,
                " didn't match because it does not initialize ",
                passedThingArticle,
                " ",
                passedThing,
                " which expects to be initialized:");
      } else {
        wr.note(fn->id(),
                candidateDescription,
                ...
                " ",
                passedThing,
                " couldn't be passed to ",
                expectedThingArticle,
                " ",
                expectedThing,
                ":");
      }
      ...
      if (candidate.formalReason() == resolution::FAIL_VARARG_TQ_MISMATCH) {
        ...
        wr.message("The variable-argument ",
                   expectedThing,
                   " ",
                   formalName,
                   ", which contains a type query, was used with ",
                   passedThing,
                   "s of "
                   "incompatible types '",
                   actuals[0].first,
                   ...
                   actuals[1].first,
                   "'.");
        wr.message("Variable-argument ",
                   expectedThing,
                   "s with type queries can only accept ",
                   passedThing,
                   "s of a single type.");
        ...
        wr.message("The instantiated type of ",
                   expectedThing,
                   " ",
                   formalName,
                   " does not allow ",
                   passedThing,
                   "s of type '",
                   badPass.actualType().type(),
                   "'.");
      } else if (badSplitInit) {
        auto formalKind = badPass.formalType().kind();
        auto actualName = "'" + actualExpr->toIdentifier()->name().str() + "'";

        if (explainedSplitInitsForActuals.insert(candidate.actualIdx())
              .second) {
          wr.note(offendingActual->id(),
                  "The actual ",
                  actualName,
                  " expects to be split-initialized because it is declared "
                  "with a generic type and no initialization expression here:");
          wr.codeForDef(offendingActual);
          wr.note(actualExpr,
                  "The call to '",
                  ci.name(),
                  "' occurs before any valid initialization points:");
          wr.code(actualExpr, {actualExpr});
          actualPrinted = true;
          wr.message("The call to '",
                     ci.name(),
                     "' cannot initialize ",
                     actualName,
                     " because only 'out' formals can be used to "
                     "split-initialize. However, ",
                     actualName,
                     " is passed to formal ",
                     formalName,
                     " which has intent '",
      ...
      if (formalReason == resolution::FAIL_INCOMPATIBLE_NILABILITY) {
        auto formalDec =
          badPass.formalType().type()->toClassType()->decorator();
        auto actualDec =
          badPass.actualType().type()->toClassType()->decorator();

        wr.message("The ",
                   expectedThing,
                   " expects a ",
                   nilabilityStr(formalDec),
                   " class, "
                   "but the ",
                   passedThing,
                   " is ",
                   nilabilityStr(actualDec),
                   ".");
        ...
        wr.message("A class with '",
                   actualMgr,
                   "' management cannot be passed to ",
                   expectedThingArticle,
                   " ",
                   expectedThing,
                   " with '",
        ...
        wr.message("Formals with kind '",
                   badPass.formalType().kind(),
                   "' expect the ",
                   passedThing,
                   " to be a subtype, but '",
                   badPass.actualType().type(),
                   "' is not a subtype of '",
                   badPass.formalType().type(),
                   "'.");
        ...
        wr.message("A tuple with ",
                   actualTup->numElements(),
                   " elements cannot be passed to a tuple formal with ",
                   formalTup->numElements(),
                   " elements.");
        ...
        wr.message("The 'ref' intent requires the ",
                   expectedThing,
                   " and ",
                   passedThing,
      ...
      bool errorInInitial = candidate.initialForErr()
                              ->formalsErroredBitmap()[candidate.formalIdx()];
      ...
      if (candidate.formalIdx() < candidate.untypedForErr()->numFormals()) {
        wr.note(candidate.idForErr(),
                "call resolution was not completed because ",
                extraText,
                expectedThing,
                " ",
                candidate.formalIdx() + 1,
                " of this candidate was ill-formed:");
        wr.code(candidate.idForErr(),
                {candidate.untypedForErr()->formalDecl(candidate.formalIdx())});
      } else {
        wr.note(candidate.idForErr(),
                "call resolution was not completed because the 'where' clause "
                "for this candidate was ill-formed:");
        wr.codeForDef(candidate.idForErr());
      }
      wr.message("For correctness reasons, candidate selection and "
                 "disambiguation cannot proceed if any of the candidates "
                 "is ill-formed.");
      break;
    } else if (reason == resolution::FAIL_NO_DEFAULT_VALUE_FOR_GENERIC_FIELD) {
      CHPL_ASSERT(candidate.untypedForErr() && candidate.initialForErr());
      CHPL_ASSERT(candidate.formalIdx() >= 0 &&
                  candidate.formalIdx() <
                    candidate.initialForErr()->numFormals());
      ...
      wr.note(candidate.idForErr(),
              ...
              candidate.formalIdx() + 1,
              fieldName,
              " had a generic type expression:");
      wr.code(candidate.idForErr(), {formal});
      wr.note(formal,
              "currently, calls to initializers cannot omit arguments for "
              "fields with generic type expressions.");
      ...
      if (auto fn = candidate.initialForErr()) {
        ...
        if (fa.failingActualIdx() != -1 && fa.failingFormalIdx() == -1) {
          ...
          wr.note(idForErr,
                  candidateDescription,
                  " didn't match because it expects ",
                  numFormals,
                  " ",
                  passedThing,
                  usePlural,
      ...
      if (allActualsLookLikeIndexing(ci)) {
        wr.message("");
        wr.note(
          idForErr, "the type '", ci.name(), "' does not support indexing:");
        if (!candidateId.isEmpty()) {
          wr.codeForLocation(candidateId);
        }
        wr.message("Only tuple types can be indexed, but '",
                   ci.name(),
                   "' is not a tuple type.");
        wr.message("If you were trying to invoke '",
                   ci.name(),
                   "' as a type constructor, note that only records and "
                   "classes have type constructors.");
      } else {
        wr.message("");
        wr.note(idForErr,
                "could not invoke the non-existent type constructor for type '",
                ci.name(),
                "':");
        if (!candidateId.isEmpty()) {
          wr.codeForLocation(candidateId);
      ...
      if (reason == resolution::FAIL_VARARG_MISMATCH) {
        ...
        reasonStr = "it did not return a type as required from associated type "
                    "procedures:";
      }
  ...
  if (printCount < rejected.size()) {
    wr.message("");
    wr.note(locationOnly(anchorId),
            "omitting ",
            rejected.size() - printCount,
            " more candidates that didn't match.");
  }
}
...
void ErrorInterfaceAmbiguousFn::write(ErrorWriterBase& wr) const {
  ...
  auto candidate =
    std::get<std::vector<const resolution::TypedFnSignature*>>(info_);

  wr.heading(kind_,
             type_,
             implPoint,
             "unable to disambiguate candidates for function '",
             fn->name(),
             "'.");
  wr.codeForDef(fn->id());
...
void ErrorInterfaceInvalidIntent::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             fnTemplate->untyped()->name(),
             "' has mismatched return intent.");
  ...
  wr.code<ID>(implPoint, {implPoint});
  wr.note(fnReal->id(),
          "the provided candidate does not have a matching return intent:");
...
void ErrorInterfaceMissingAssociatedType::write(ErrorWriterBase& wr) const {
  ...
  auto actualDecls =
    std::vector<const uast::VarLikeDecl*>(rejected.size(), nullptr);
  wr.heading(kind_,
             type_,
             implPoint,
             "unable to find matching candidates for associated type '",
             var->name(),
             "'.");
  ...
  wr.note(implPoint, "while checking the implementation point here:");
  wr.code<ID>(implPoint, {implPoint});
  wr.message("Associated types are resolved as 'type' calls on types "
             "constrained by the interface.");
  printRejectedCandidates(
    wr,
    implPoint,
    ci,
    rejected,
    ...
    "a",
    "formal",
    [](int) -> const uast::AstNode* { return nullptr; },
    actualDecls);
}
...
void ErrorInterfaceMissingFn::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             implPoint,
             "unable to find matching candidates for function '",
             fn->untyped()->name(),
             "'.");
  wr.codeForDef(fn->id());
  wr.message("Required by the interface '", interface->name(), "':");
...
void ErrorInterfaceMultipleImplements::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "' found.");
  wr.message(
    "While analyzing the definition of type '", ad->name(), "', defined here:");
  wr.codeForDef(ad);
  wr.note(implPoint,
          "the interface '",
          interface->name(),
          "' is first implemented here:");
  wr.code<ID>(implPoint, {implPoint});
...
void ErrorInterfaceNaryInInherits::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             interface->name(),
             "' in inheritance expression as it is not a unary interface.");
  wr.message(
    "While analyzing the definition of type '", ad->name(), "', defined here:");
  wr.codeForDef(ad);
  wr.note(implPoint,
          "found the interface '",
          interface->name(),
          "' in an inheritance list here:");
  wr.code<ID>(implPoint, {implPoint});
  wr.note(interface->id(),
          "However, the interface '",
          interface->name(),
          "' is defined to be a non-unary interface here:");
  wr.codeForDef(interface->id());
  wr.message("Only unary interfaces (those with a single type parameter like "
             "'Self') can be used in inheritance expressions.");
  wr.message("To implement n-ary interfaces, consider using a standalone "
...
void ErrorInterfaceReorderedFnFormals::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "candidate for function '",
             fnTemplate->untyped()->name(),
             "' does not have the same order of formal names.");
  wr.codeForDef(fnTemplate->id());
  wr.message("Required by the interface '", interface->name(), "':");
  wr.codeForDef(interface->id());
  wr.note(implPoint, "while checking the implementation point here:");
  ...
  wr.note(
    fnReal->id(),
    "the provided candidate defined here does not have matching formals:");
  wr.codeForDef(fnReal->id());
}
...
void ErrorInvalidClassCast::write(ErrorWriterBase& wr) const {
  ...
  if (prim == uast::primtags::PRIM_TO_NILABLE_CLASS_CHECKED && !type.isType()) {
    wr.heading(kind_,
               type_,
               primCall,
               "cannot apply '?' operator to a non-type argument.");
  ...
  if (primitiveConversion) {
    wr.heading(kind_,
               ...
               ".");
    wr.message(
      "Only classes or class-like types are supported by this conversion.");
  } else {
...
void ErrorInvalidContinueBreakTarget::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, stmt, "invalid target for ", stmtType, " statement.");
  wr.code(stmt, {targetNode});
  wr.message("A ",
             stmtType,
             " statement can only refer to a loop. This is done by using the "
             "loop's label.");
...
void ErrorInvalidDomainCall::write(ErrorWriterBase& wr) const {
  ...
  wr.message("The 'domain' keyword should be used with a valid domain type "
             "expression.");
  ...
  if (fnCall->numActuals() == 0) {
    ...
    if (!qt.isType() && !(qt.type() && qt.type()->isIntType())) {
      wr.message("However, the first actual was ",
                 decayToValue(qt),
    ...
    if (fnCall->numActuals() >= 2) {
      ...
      if (!idxTypeQt.isType()) {
        ...
        wr.message("However, the second actual ('idxType') was ",
                   decayToValue(idxTypeQt),
                   " rather than a type as required.");
...
void ErrorInvalidImplementsActual::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             impl,
             "invalid use of 'implements' with an actual that is ",
             qt,
             ".");
...
void ErrorInvalidImplementsArity::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    ...
    type_,
    impl,
    "wrong number of actuals in 'implements' statement for interface '",
    interface->name(),
    "'.");
  wr.codeForLocation(impl);
}
...
void ErrorInvalidImplementsInterface::write(ErrorWriterBase& wr) const {
  ...
  wr.codeForLocation(impl);
  wr.message(
    "The statement attempts to implement ", qt, ", which is not an interface.");
}

static void
printErrorInvalidDomainKeywordCall(ErrorWriterBase& wr,
                                   ErrorBase::Kind kind,
                                   ErrorType errType,
                                   const char* keyword,
  ...
  wr.message("The '",
             keyword,
             "' keyword should be used with a domain: '",
             keyword,
             "(D)'.");
  ...
  if (fnCallActuals->numActuals() == 0) {
    ...
    wr.message("However, '",
               keyword,
               "' here is not called with a domain argument, but with ",
               decayToValue(type),
               ".");
...
void ErrorInvalidIndexCall::write(ErrorWriterBase& wr) const {
  ...
  printErrorInvalidDomainKeywordCall(
    wr, kind_, type_, "index", fnCall, fnCall, type);
}
...
void ErrorInvalidSubdomainCall::write(ErrorWriterBase& wr) const {
  ...
  printErrorInvalidDomainKeywordCall(
    wr, kind_, type_, "subdomain", fnCall, fnCall, type);
}
...
void ErrorInvalidSparseSubdomainCall::write(ErrorWriterBase& wr) const {
  ...
  printErrorInvalidDomainKeywordCall(
    wr, kind_, type_, "sparse subdomain", fnCallLocation, fnCallActuals, type);
}
...
void ErrorInvalidNewTarget::write(ErrorWriterBase& wr) const {
  ...
  if (auto primType = type.type()->toPrimitiveType()) {
    wr.heading(kind_,
               type_,
               newExpr,
               "invalid use of 'new' on primitive '",
               primType,
               "'");
    ...
    wr.heading(kind_,
               type_,
               newExpr,
               "invalid use of 'new' on enum type '",
               enumType->name(),
               "'");
    ...
    wr.heading(kind_,
               type_,
               newExpr,
               "invalid use of 'new' on tuple type '",
               tupleType,
               "'");
  } else {
    wr.heading(kind_,
...
void ErrorInvalidSuper::write(ErrorWriterBase& wr) const {
  ...
  if (recordType) {
    wr.heading(kind_,
               type_,
               superExpr,
               "invalid use of 'super' in record '",
               recordType->name(),
               "'.");
  } else {
  ...
  if (recordType) {
    ...
    wr.message("If you meant to declare '",
               recordType->name(),
               "' as a class ",
               "instead, you can do that by writing 'class ",
               recordType->name(),
               "' instead of 'record ",
               recordType->name(),
               "'.");
...
void ErrorInvalidTupleCast::write(ErrorWriterBase& wr) const {
  ...
  wr.codeForLocation(astForErr);
  wr.note(astForErr,
          "specifically, cannot cast element type '",
          fromType,
          "' to '",
          toType,
          "'.");
...
void ErrorIteratorsInOtherScopes::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    ...
    type_,
    node,
    "found potentially compatible iterator overloads in other scopes.");
  wr.message("While resolving the call to iterator '", name, "' here:");
  wr.codeForLocation(call);
  if (candidate) {
    wr.message(
      "The following iterator was determined to match the given call:");
...
void ErrorLoopLabelOutsideBreakOrContinue::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             label,
             "invalid reference to loop label outside of a 'break' or "
             "'continue' statement.");
  wr.message(
...
void ErrorMemManagementNonClass::write(ErrorWriterBase& wr) const {
  ...
  if (record) {
    ...
    wr.message("Consider removing the '",
               uast::New::managementToString(newCall->management()),
               "' keyword to fix this error, or defining '",
               record->name(),
               "' as a class.");
  }
}
...
void ErrorMissingFormalInstantiation::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             call,
             "call does not provide enough type information for a complete "
             "instantiation.");
  wr.code(call, {call});
  for (auto& formal : genericFormals) {
    ...
    if (auto fieldDecl = decl->toVariable();
        fieldDecl && fieldDecl->isField()) {
    ...
    wr.note(decl,
            ...
            " has generic type '",
            qt.type(),
            "', but is expected to have a concrete type.");
    wr.codeForDef(decl);
      ...
      if (auto ct = qt.type()->toClassType()) {
        if (ct->decorator().isUnknownManagement() && ct->basicClassType()) {
          wr.note(decl,
                  "one reason that ",
                  formalName,
                  " is generic is that it doesn't have a specified memory "
                  "management strategy like 'owned', 'shared' or 'unmanaged'.");
          wr.message("Consider explicitly specifying a memory management "
                     "strategy, or adding a new type parameter to explicitly "
                     "make the formal generic.");
        }
      }
    }
...
void ErrorMismatchedInitializerResult::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             node,
             "final result of initialization does not match the initial type.");
  wr.message("In the following initialization: ");
  wr.code(justOneLine(node), {node});
  wr.message(
    "The initializer was invoked with an instantiated type '", initialCT, "'.");
  wr.message(
    "However, the initializer resulted in a value of type '", finalCT, "'.");
...
void ErrorMultipleInheritance::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             theClass,
             "invalid use of multiple inheritance in class '",
             theClass->name(),
...
void ErrorNestedClassFieldRef::write(ErrorWriterBase& wr) const {
  ...
  wr.note(outerDecl,
          "however, the identifier refers to a field of an enclosing ",
          outerName,
          " '",
          getName(outerDecl),
          "', declared here:");
...
void ErrorNoMatchingCandidates::write(ErrorWriterBase& wr) const {
  ...
  auto& ci = std::get<resolution::CallInfo>(info_);
  auto& rejected =
    std::get<std::vector<resolution::ApplicabilityResult>>(info_);
  ...
  printRejectedCandidates(
    ...
    "a",
    "formal",
    [call, &ci](int idx) -> const uast::AstNode* {
...
void ErrorNoMatchingSuper::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    ...
    type_,
    node,
    "failed to resolve implicit call to 'super.init': no matching candidates.");
  wr.code(node);
  wr.message("The call to 'super.init' was triggered by the use of fields from "
             "the parent type.");
  ...
  wr.message("In order to use fields from a parent class, the parent's "
             "initializer must have been called.");
  wr.message(
    "Chapel attempts to automatically call a zero-argument initializer on the "
    "parent class if an explicit call is not present.");
  ...
  printRejectedCandidates(
    ...
    "a",
    "formal",
    [](int idx) -> const uast::AstNode* { return nullptr; },
    actualDecls);

  wr.message("If the parent type's initializer has formals, consider invoking "
             "it explicitly.");
}
...
void ErrorNonClassInheritance::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, ad, "attempt for non-class type to inherit from a type.");
  wr.message(
    "While analyzing the definition of type '", ad->name(), "', defined here:");
  wr.codeForDef(ad);
  wr.note(inheritanceExpr,
          "found an inheritance expression referring to type '",
          type,
          "' here:");
  wr.code(inheritanceExpr, {inheritanceExpr});
}

static void printTheseResults(ErrorWriterBase& wr,
  ...
  while (currentTr) {
    ...
    if (firstLine) {
      start = std::string("The ") + iterGroupStr +
    ...
    if (reason == TheseResolutionResult::THESE_NOT_ATTEMPTED) {
      ...
      wr.message(start,
                 "loop expressions with more than one zippered iterand do not "
                 "have standalone iterators",
                 end);
    } else if (reason ==
               TheseResolutionResult::THESE_FAIL_NO_PROMO_STANDALONE) {
      wr.message(start,
                 "promoted expressions with more than one zippered iterand do "
                 "not have standalone iterators",
                 end);
    } else if (reason == TheseResolutionResult::THESE_FAIL_SERIAL_LOOP_EXPR) {
      wr.message(
        start, "the loop expression iterated over is explicitly serial", end);
    } else if (reason ==
               TheseResolutionResult::THESE_FAIL_NO_ITERATOR_WITH_TAG) {
      wr.message(
        start, "an iterator of that kind was not defined for the type", end);
    } else if (reason ==
               TheseResolutionResult::THESE_FAIL_LEADER_FOLLOWER_MISMATCH) {
      wr.message(start,
                 "the yield type of the leader iterator does not match the "
                 "type of the 'followThis' argument",
                 end);
    } else if (reason ==
               TheseResolutionResult::THESE_FAIL_ZIPPERED_ARG_FAILED) {
    ...
    } else if (reason == TheseResolutionResult::
                           THESE_FAIL_PROMOTION_TYPE_YIELD_MISMATCH) {
      wr.message(start,
                 "the claimed scalar type for promotion does not match the "
                 "type yielded from the iterator",
                 end);
    }
  ...
  for (auto& rr : resolutionResults) {
    ...
    if (iterKind == uast::Function::LEADER) {
      failedLeader = true;
               ...
               tr.reason() ==
                 resolution::TheseResolutionResult::THESE_NOT_ATTEMPTED &&
               failedLeader) {
...
void ErrorNoTypeForEnumElem::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    ...
    type_,
    enumAst,
    "cannot pick single numeric type to represent the elements of enum '",
    enumAst->name(),
    "'");
  wr.note(signedElt,
          "the constant '",
          ...
          ", ",
          signedQt,
          ", which requires a signed integer type.");
  ...
  wr.note(unsignedElt,
          ...
          ", ",
          unsignedQt,
          ", which requires an unsigned integer type.");
...
void ErrorPhaseTwoInitMarker::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "use of '",
             markerType,
             "' after type has been initialized.");
  wr.code(node, {node});

  auto previousMarker = others.at(0);
  wr.note(previousMarker,
          "the type was previously marked as initialized here:");
...
void ErrorPotentiallySurprisingShadowing::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             id,
             "potentially surprising shadowing for '",
             name.c_str(),
             "'");
  wr.code<ID, ID>(id, {id});
  ...
  if (result.numIds() > 0 && shadowed.numIds() > 0) {
    ...
    if (needsIntroText) {
      wr.note(locationOnly(otherId),
              "but, there is a shadowed symbol '",
              from,
              "' defined here:");
    } else {
...
void ErrorProcDefExplicitAnonFormal::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             formal,
             "formals in a procedure definition must "
             "be named");
  wr.code(fn, {formal});
}
...
void ErrorProcTypeUnannotatedFormal::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             formal,
             "unannotated formal is ambiguous in this "
             "context");
  wr.code(sig, {formal});
...
// if one is found.
static bool
firstIdFromDecls(const resolution::MatchingIdsWithName& matches,
...
void ErrorRaceyOutInoutInPromotion::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             ast,
             "cannot promote function '",
             c.fn()->untyped()->name(),
             "' while keeping the formal '",
             intent,
             " ",
             formalName,
             "' scalar");
  ...
  wr.message("'",
             intent,
             "' actuals will be written to by each iteration of the promoted "
             "function, which can lead to races.");
...
void ErrorRecursionFieldDecl::write(ErrorWriterBase& wr) const {
  ...
  if (auto vld = ast->toVarLikeDecl()) {
    wr.heading(kind_,
               type_,
               ast,
               "encountered recursion while resolving field '",
               vld->name(),
               "' of type '",
               ad->name(),
               "':");
    ...
    wr.heading(kind_,
               type_,
               ast,
               "encountered recursion while resolving a field"
               "of type '",
               ad->name(),
               "':");
  }
...
void ErrorRedefinition::write(ErrorWriterBase& wr) const {
  ...
  if (anchorToDef) {
    ...
    wr.heading(
      kind_, type_, id, "'", name, "' has multiple definitions in this scope.");
  }
...
void ErrorReductionAssignNonIdentifier::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, opCall, "invalid reduction assignment to non-identifier:");
  wr.message("The left-hand side of a 'reduce=' statement must be an "
             "identifier referring to a variable with a 'reduce' intent.");
...
void ErrorReductionAssignNotReduceIntent::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             opCall,
             "invalid reduction assignment to variable declared outside of "
             "'reduce' intent:");
  wr.codeForLocation(opCall);
  wr.message("The left-hand side of a 'reduce=' statement must be an "
             "identifier referring to a variable with a 'reduce' intent.");
  wr.message("However, the left-hand side is declared (outside a 'reduce' "
             "intent) here:");
  wr.codeForDef(lhs);
  wr.message("The 'reduce=' operator uses the 'reduce' intent to determine the "
             "reduction operation.");
}
...
void ErrorReductionAssignInvalidRhs::write(ErrorWriterBase& wr) const {
  ...
  wr.codeForLocation(opCall);
  wr.message(
    "The right-hand side of the 'reduce=' statement refers to a variable '",
    reduceIntent->name(),
    "' declared '",
    reduceIntent->op(),
    " reduce' here:");
  wr.codeForDef(reduceIntent);
  wr.message("This shadows the declaration of '",
             reduceIntent->name(),
             "' with type '",
             ci.actual(1).type().type(),
             "' here:");
  wr.codeForDef(shadowed);
  wr.message("However, the reduction '",
             reduceIntent->op(),
             "' does not support an assignment from ",
             decayToValue(ci.actual(2).type()),
             ".");
...
void ErrorReductionIntentChangesType::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, reduceIntent, "cannot use non-closed reduction in intent:");
  wr.codeForLocation(reduceIntent);
  wr.message("A reduction is closed if it produces values of the same type as "
             "the values it operates on.");
  wr.message("The reduce intent shadows the declaration of '",
             reduceIntent->name(),
             "' with type '",
             oldType.type(),
             "' here:");
  ...
  wr.message("However, the reduction '",
             ...
             newType.type(),
             "'.");
  wr.message("The reduced type of a variable cannot change, as the final "
             "reduced value is assigned back into the variable.");
}

static const uast::AstNode*
getReduceOrScanOp(const uast::AstNode* reduceOrScan) {
...
void ErrorReductionInvalidName::write(ErrorWriterBase& wr) const {
  ...
  wr.message("Identifiers on the left of the 'reduce' expression are applied "
             "to the type of the iterator's elements ('",
             iterType.type(),
             "' "
             "in this case)");
  wr.message(
    "Is '", name, "(", iterType.type(), ")' a valid reduction operation?");
}
...
void ErrorReductionNotReduceScanOp::write(ErrorWriterBase& wr) const {
  ...
  wr.message("The operation must be a type extending 'ReduceScanOp', but "
             ...
             actualType);
  if (actualClassType) {
    wr.message("Did you mean for class '",
               actualClassType,
...
void ErrorSelfDefinition::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "' to define itself.");
  wr.message(
    "The following statement uses the variable '", name, "' to define itself:");
  wr.code(decl);
...
void ErrorSplitInitMismatchedConditionalTypes::write(
  ...
  if (node->isConditional()) {
    ...
    wr.heading(kind_,
               type_,
               var,
               "mismatched types for split-initialization of '",
               var->name(),
               "' in conditional branches.");
    wr.note(
      cond->thenBlock(), "initialized with ", thenType, " in 'then' branch");
    wr.note(
      cond->elseBlock(), "initialized with ", elseType, " in 'else' branch");
    wr.message("Types of different initialization parts for split-initialized "
    ...
    wr.note(branch1, "Initialized with ", thenType, " in one branch");
    wr.note(branch2, "Initialized with ", elseType, " in another branch");
    wr.message("Types of different initialization parts for split-initialized "
               "declarations must exactly match");
  }
}
...
void ErrorSuperFromTopLevelModule::write(ErrorWriterBase& wr) const {
  ...
  auto useOrImportStr = (useOrImport == resolution::VIS_USE) ? "use" : "import";
  wr.heading(kind_,
             type_,
             use,
             "invalid use of 'super' in '",
             useOrImportStr,
             "'; '",
             mod->name().c_str(),
...
static const char* genericityToString(types::Type::Genericity genericity) {
  switch (genericity) {
    case types::Type::GENERIC: return "generic";
    case types::Type::GENERIC_WITH_DEFAULTS: return "generic with defaults";
    case types::Type::MAYBE_GENERIC: return "maybe generic";
    case types::Type::CONCRETE: return "concrete";
    default: CHPL_ASSERT(false && "shouldn't happen"); return "";
  }
}
...
void ErrorSyntacticGenericityMismatch::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             var->name(),
             "' appears to be ",
             genericityToString(syntacticGenericity),
             " in the program's syntax, but it has been inferred to be ",
             type,
             ", which is ",
             genericityToString(actualGenericity),
             ".");
...
void ErrorFieldWithGenericManagement::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, decl, "field is declared with generic memory management.");
  wr.codeForLocation(decl);
  wr.note(decl, "Consider adding 'owned', 'shared', or 'borrowed'.");
  wr.note(decl,
          "If generic memory management is desired, use a 'type' field to "
...
void ErrorTertiaryUseImportUnstable::write(ErrorWriterBase& wr) const {
  ...
  auto useOrImport = std::get<resolution::VisibilityStmtKind>(info_);
  auto useOrImportStr =
    (useOrImport == resolution::VIS_USE) ? "a 'use'" : "an 'import'";
  wr.heading(
    kind_,
    type_,
    clause,
  ...
  wr.code(clause, {node});
  wr.message(
    "The type '", name, "' is not defined in '", searchedScope->name(), "'.");
}
...
void ErrorThrowInNonThrowingFunction::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, throwNode, "cannot throw in a non-throwing function");
  wr.code(throwNode, {throwNode});
  wr.message("Either put the throw inside a try block, or add 'throws' to the "
...
void ErrorTryNoCatchAll::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, tryNode, "try without a catchall in a non-throwing function");
...
void ErrorTupleDeclAssignMismatchedElems::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             ast,
             "tuple size mismatch in split tuple ",
             declOrAssign,
             ".");
  wr.code(ast);
  wr.message("The left-hand side of the ",
             declOrAssign,
             " expects a ",
             lhsSize,
             "-tuple, but the right-hand side is a ",
             rhsType->numElements(),
             "-tuple, '",
             rhsType,
             "'.");
...
void ErrorTupleDeclNotTuple::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             decl,
             "value of non-tuple type '",
             type,
             "' cannot be split using a tuple "
             "declaration.");
  wr.message("In the following tuple declaration:");
  wr.code(decl);
  wr.message("the initialization expression has type '",
             type,
...
void ErrorTupleExpansionNamedArgs::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             fnCall,
             "tuple expansion cannot be used to pass "
...
void ErrorTupleIndexOOB::write(ErrorWriterBase& wr) const {
  ...
  wr.message("In the following expression:");
  wr.code(call, {call});
  wr.message("the index value is '",
             index,
             "' but the valid indices for this",
             " tuple are in the range 0..",
             type->numElements() - 1,
             " (inclusive)");
}
...
void ErrorTupleCastSizeMismatch::write(ErrorWriterBase& wr) const {
  ...
  wr.note(node,
          ...
          fromType,
          "' has ",
          fromType->numElements(),
          " elements.");
  wr.note(node,
          "however, the target tuple '",
          toType,
          "' has ",
          toType->numElements(),
          " elements.");
...
void ErrorUseImportMultiplyDefined::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(
    kind_, type_, secondOccurrence, "'", symbolName, "' is multiply defined.");
...
void ErrorUseImportMultiplyMentioned::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_, type_, secondOccurrence, "'", symbolName, "' is repeated.");
...
void ErrorUseImportTransitiveRename::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_, type_, secondRename, "'", middle, "' is repeated.");
...
void ErrorUseImportUnknownMod::write(ErrorWriterBase& wr) const {
  ...
  if (!improperMatches.empty()) {
    wr.message("The following declarations are not covered by the '",
               useOrImport,
    ...
    for (auto ast : improperMatches) {
      ...
      if (tag == uast::asttags::AstTag::Module) {
        ...
        wr.note(locationOnly(improperId),
                "however, a full path or an explicit relative ",
                useOrImport,
    ...
    if (explainRelativeImport) {
      wr.message("");
      wr.note(id,
              "For non-root-level modules, please specify the full path "
...
void ErrorUseImportUnknownSym::write(ErrorWriterBase& wr) const {
  ...
  wr.message("Searching in the scope of ",
             whatIsSearched,
             " '",
             searchedScope->name(),
             "':");
...
void ErrorUseOfLaterVariable::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             type_,
             stmt,
             "statement references variable '",
             name,
             "' before it is defined.");
  wr.message("The following statement references the variable '",
             name,
...
void ErrorVariableWithoutInitOrType::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "variable '",
             name,
             "' is declared without an initializer or type.");
  if (!useOf.isEmpty()) {
    wr.note(useOf,
...
void ErrorValueUsedAsType::write(ErrorWriterBase& wr) const {
  ...
  wr.heading(kind_,
             ...
             "type specifier is ",
             type,
             ", but it was expected to be a type.");
