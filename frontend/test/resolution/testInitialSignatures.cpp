...
template <typename F>
static void
testInitialFormalOfF(const char* prog, int formalIdx, F&& predicate) {
...
static void test1() {
  testInitialFormalOfF(R""""(
    ...
    proc f(x: id(numeric)) {}
                       ...
                       0,
                       [](Context* context, const QualifiedType& qt) {
...
static void test5() {
  testInitialFormalOfF(
    R""""(
    proc id(type t) type do return t;
    proc f(type x: numeric, y: id(x)) {}
  )"""",
    1,
    [](Context* context, const QualifiedType& qt) { assert(qt.isUnknown()); });
}
...
static void test7() {
  testInitialFormalOfF(
    ...
    [](Context* context, const QualifiedType& qt) {
      ...
      ensureSubs(
        context,
        qt.type()->toRecordType(),
        {{"wraps",
          QualifiedType(QualifiedType::TYPE, AnyNumericType::get(context))}});
    });
}
