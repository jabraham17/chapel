...
inline size_t hash(const std::string& s) {
  uint64_t seed = FNV_offset_basis;
  for (char c : s) seed = (seed ^ static_cast<unsigned char>(c)) * FNV_prime;
  return (size_t)seed;
}
...
template <typename... Ts> inline size_t hash(const Ts&... data) {
  size_t ret = 0;
  for (auto h : {hash(data)...}) ret = hash_combine(ret, h);
  return ret;
}
...
template <typename... Ts> inline size_t hash(const std::tuple<Ts...>& tuple) {
  return hash_tuple_impl(tuple, std::index_sequence_for<Ts...>{});
}
static inline size_t hash(const std::tuple<>& tuple) { return 0; }

template <typename T> inline size_t hashVector(const std::vector<T>& key) {
  size_t ret = 0;
...
template <typename T> inline size_t hashSet(const std::set<T>& key) {
  size_t ret = 0;
  for (const auto& elt : key) {
    ret = hash_combine(ret, hash(elt));
  }
...
template <typename T> inline size_t hashOwned(const chpl::owned<T>& key) {
  size_t ret = 0;
  if (key) {
...
template <typename T> inline size_t hashOptional(const chpl::optional<T>& key) {
  size_t ret = 0;
...
template <typename Map> inline size_t hashMap(const Map& key) {
  size_t ret = 0;
...
template <typename T> struct hasher<std::set<T>> {
  size_t operator()(const std::set<T>& key) const { return chpl::hashSet(key); }
};
