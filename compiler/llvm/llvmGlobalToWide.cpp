...
AllocaInst*
makeAlloca(llvm::Type* type, const char* name, Instruction* insertBefore) {
  ...
  if (insertBefore) {
    tempVar = new AllocaInst(
      type, DL.getAllocaAddrSpace(), name, getInsertPosition(insertBefore));
    trackLLVMValue(tempVar);
  } else {
    tempVar = new AllocaInst(type, DL.getAllocaAddrSpace(), name, entryBlock);
    trackLLVMValue(tempVar);
  }
...
template <typename BaseTy,
          std::enable_if_t<std::is_base_of_v<Function, BaseTy> ||
                             std::is_base_of_v<CallBase, BaseTy>,
                           bool> = true>
...
Instruction*
createRaddr(GlobalToWideInfo* info, Value* widePtr, Instruction* insertBefore) {
  // Assuming widePtr is a struct wide-pointer, extract the address
  assert(widePtr->getType()->isStructTy());

  Instruction* ret = ExtractValueInst::Create(
    widePtr, wideAddrGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ret);
  return ret;
}
Instruction* createRlocale(GlobalToWideInfo* info,
                           Value* widePtr,
  ...
  Instruction* ret = ExtractValueInst::Create(
    widePtr, wideLocaleGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ret);
  return ret;
}
Instruction*
createRnode(GlobalToWideInfo* info, Value* widePtr, Instruction* insertBefore) {
  // Assuming widePtr is a struct wide-pointer, extract the address
  assert(widePtr->getType()->isStructTy());

  Instruction* ret = ExtractValueInst::Create(
    widePtr, wideNodeGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ret);
  return ret;
}
Instruction* createWideAddr(GlobalToWideInfo* info,
  ...
  Instruction* locSet = InsertValueInst::Create(
    undefWidePtr, localeId, wideLocaleGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(locSet);

  Instruction* ptrSet = InsertValueInst::Create(
    locSet, addr, wideAddrGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ptrSet);

  return ptrSet;
}

Value* createWideBitCast(GlobalToWideInfo* info,
                         Value* widePtr,
                         Type* widePtrType,
                         Instruction* insertBefore) {
  ...
  Value* loc = ExtractValueInst::Create(
    widePtr, wideLocaleGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(loc);

  Value* ptr = ExtractValueInst::Create(
    widePtr, wideAddrGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ptr);
  ...
  Value* cast = CastInst::CreatePointerCast(
    ptr, undefLocPtr->getType(), "", getInsertPosition(insertBefore));
  trackLLVMValue(cast);

  Instruction* locSet = InsertValueInst::Create(
    undef, loc, wideLocaleGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(locSet);

  Instruction* ptrSet = InsertValueInst::Create(
    locSet, cast, wideAddrGEP, "", getInsertPosition(insertBefore));
  trackLLVMValue(ptrSet);
...
Instruction*
createStoreLoadCast(Value* fromValue, Type* toType, Instruction* insertBefore) {

  Function* inFunc = insertBefore->getParent()->getParent();
  const DataLayout& DL = inFunc->getParent()->getDataLayout();

  Type* allocType = toType;
  if (DL.getTypeStoreSize(fromValue->getType()) > DL.getTypeStoreSize(toType))
  ...
  if (allocAsFrom->getType() != fromPtrType) {
    allocAsFrom = CastInst::CreatePointerCast(
      alloc, fromPtrType, "", getInsertPosition(insertBefore));
    trackLLVMValue(allocAsFrom);
  }
  Value* allocAsNew = alloc;
  if (allocAsNew->getType() != newPtrType) {
    allocAsNew = CastInst::CreatePointerCast(
      alloc, newPtrType, "", getInsertPosition(insertBefore));
    trackLLVMValue(allocAsNew);
  }

  Instruction* store =
    new StoreInst(fromValue, allocAsFrom, getInsertPosition(insertBefore));
  trackLLVMValue(store);
  Instruction* load =
    new LoadInst(toType, allocAsNew, "", getInsertPosition(insertBefore));
  trackLLVMValue(load);
...
struct GlobalTypeFixer : ValueMapTypeRemapper {
  ...
  CallInst* callGlobalToWideFn(Value* globalPtr, Ty insertBefore) {
    ...
    CallInst* call =
      CallInst::Create(fn, local_args, "", getInsertPosition(insertBefore));
    trackLLVMValue(call);
    return call;
  }
  template <typename Ty,
            std::enable_if_t<is_Inst_or_BBiterator<Ty>(), bool> = true>
  CallInst*
  callWideToGlobalFn(Value* widePtr, Type* globalTy, Ty insertBefore) {
  ...
  void fixInstruction(Instruction* insn) {
    ...
    switch (insn->getOpcode()) {
      ...
      case Instruction::InsertElement: {
        ...
        if (insertType->isPointerTy() &&
            insertType->getPointerAddressSpace() == info->globalSpace) {
          auto newVecType =
            convertTypeGlobalToWide(&M, info, vecVal->getType());
          auto newVecVal = createStoreLoadCast(vecVal, newVecType, insn);

          auto newInsertVal = createStoreLoadCast(
            insertVal, IntegerType::getInt128Ty(M.getContext()), insn);

          auto newInsn = InsertElementInst::Create(
            newVecVal, newInsertVal, idx, "", getInsertPosition(insn));
          trackLLVMValue(newInsn);
          auto convertBack =
            createStoreLoadCast(newInsn, vecVal->getType(), insn);
      ...
      case Instruction::ExtractElement: {
        ...
        if (vecElmType->isPointerTy() &&
            vecElmType->getPointerAddressSpace() == info->globalSpace) {
          auto newVecType =
            convertTypeGlobalToWide(&M, info, vecVal->getType());
          auto newVecVal = createStoreLoadCast(vecVal, newVecType, insn);

          auto newInsn = ExtractElementInst::Create(
            newVecVal, idx, "", getInsertPosition(insn));
          trackLLVMValue(newInsn);
      ...
      case Instruction::ShuffleVector: {
        ...
        auto vecElmType =
          cast<VectorType>(vecVal1->getType())->getElementType();

        if (vecElmType->isPointerTy() &&
            vecElmType->getPointerAddressSpace() == info->globalSpace) {
          auto newVec1Type =
            convertTypeGlobalToWide(&M, info, vecVal1->getType());
          auto newVecVal1 = createStoreLoadCast(vecVal1, newVec1Type, insn);

          auto newVec2Type =
            convertTypeGlobalToWide(&M, info, vecVal2->getType());
          ...
          Value* newInsn;
          {
            ...
            newInsn =
              irBuilder.CreateShuffleVector(newVecVal1, newVecVal2, mask);
            trackLLVMValue(newInsn);
          }
      ...
      case Instruction::PHI: {
        ...
        for (unsigned j = 0; j < nIncoming; j++) {
          ...
          newIncoming[j] =
            callGlobalToWideFn(incomingValue, getInsertPosition(incomingEnd));
        }

        // create a new phi node with the replacement incoming values.
        PHINode* newPHI =
          PHINode::Create(wideTy, nIncoming, "", getInsertPosition(oldPHI));
        trackLLVMValue(newPHI);
        for (unsigned j = 0; j < nIncoming; j++) {
          newPHI->addIncoming(newIncoming[j], oldPHI->getIncomingBlock(j));
        }
      ...
      case Instruction::BitCast: {
        ...
        bool fromOk = srcTy->isPointerTy() || srcTy->isFunctionTy();
        bool toOK = dstTy->isPointerTy() || dstTy->isFunctionTy();
        ...
        if (!(fromOk && toOK)) {
          ...
          if (oldCast->getSrcTy()->isPointerTy() &&
            ...
            if (fromGlobal && !toGlobal) {
              ...
              // See LLVM bug 15907.
              if (!toWide) assert(0 && "casting from global to local pointer");
            } else {
              if (!fromWide)
      ...
      case Instruction::GetElementPtr: {
        ...
        if (oldGEP->getPointerAddressSpace() == info->globalSpace) {
          ...
          Type* newPointeeType =
            convertTypeGlobalToWide(&M, info, oldPointeeType);
          Type* newResultType =
            convertTypeGlobalToWide(&M, info, oldResultType);
          ...
          GetElementPtrInst* newGEP =
            GetElementPtrInst::Create(newPointeeType,
                                      raddr,
                                      inds,
                                      oldGEP->getName(),
          ...
          Instruction* outWidePtr =
            createWideAddr(info, loc, newGEP, newResultType, oldGEP);
      ...
      case Instruction::Load: {
        ...
        if (oldLoad->getPointerAddressSpace() == info->globalSpace) {
          ...
          Value* castAlloc =
            new BitCastInst(alloc, voidPtrTy, "", getInsertPosition(oldLoad));
          Value* node = createRnode(info, wAddr, oldLoad);
          Value* raddr = createRaddr(info, wAddr, oldLoad);
          Value* castRaddr =
            new BitCastInst(raddr, voidPtrTy, "", getInsertPosition(oldLoad));
          ...
          args[2] = castRaddr;
          args[3] = size;
          args[4] = createLoadStoreControl(
            M, info, oldLoad->getOrdering(), oldLoad->getSyncScopeID());

          Value* call = CallInst::Create(
            getFnType, getFn, args, "", getInsertPosition(oldLoad));
          trackLLVMValue(call);
          if (call == nullptr) assert(false && "failure creating call");

          // Now load from the alloc'd area.
          Instruction* loadedWide = new LoadInst(wLoadedTy,
                                                 alloc,
                                                 "",
                                                 oldLoad->isVolatile(),
                                                 oldLoad->getAlign(),
      ...
      case Instruction::Store: {
        ...
        if (oldStore->getPointerAddressSpace() == info->globalSpace) {
          ...
          Value* castAlloc =
            new BitCastInst(alloc, voidPtrTy, "", getInsertPosition(oldStore));
          trackLLVMValue(castAlloc);

          // Now store to the alloc'd area
          Instruction* st = new StoreInst(wValueOp,
                                          alloc,
                                          oldStore->isVolatile(),
                                          oldStore->getAlign(),
          ...
          Value* castRaddr =
            new BitCastInst(raddr, voidPtrTy, "", getInsertPosition(oldStore));
          ...
          args[3] = size;
          args[4] = createLoadStoreControl(
            M, info, oldStore->getOrdering(), oldStore->getSyncScopeID());

          Instruction* put = CallInst::Create(
            putFnType, putFn, args, "", getInsertPosition(oldStore));
          trackLLVMValue(put);
      ...
      case Instruction::PtrToInt: {
        ...
        if (ptrToInt->getPointerAddressSpace() == info->globalSpace) {
          ...
          Instruction* conv =
            createStoreLoadCast(ptr, ptrToInt->getType(), ptrToInt);
      ...
      case Instruction::IntToPtr: {
        ...
        if (intToPtr->getAddressSpace() == info->globalSpace) {
          ...
          Instruction* conv =
            createStoreLoadCast(i, intToPtr->getType(), intToPtr);
      ...
      case Instruction::Call: {
        ...
        if (calledFn && starts_with(calledFn->getName(), GLOBAL_FN)) {
          ...
          if (starts_with(calledFn->getName(), GLOBAL_FN_WIDE_TO_GLOBAL)) {
          ...
          } else if (starts_with(calledFn->getName(),
            ...
            if (extr->getType() != calledFn->getReturnType()) {
              extr = CastInst::CreatePointerCast(
                extr, wLocAddrTy, "", getInsertPosition(call));
              trackLLVMValue(extr);
            }
            ...
            Instruction* make =
              createWideAddr(info, locale, wLocAddr, wResType, call);
          ...
          if (isa<MemCpyInst>(call) || isa<MemMoveInst>(call)) {
            ...
            Value* ctl = createLoadStoreControl(
              M, info, AtomicOrdering::NotAtomic, SyncScope::SingleThread);
            ...
            if (dstSpace == info->globalSpace &&
              ...
              putget = CallInst::Create(
                putFnType, putFn, args, "", getInsertPosition(call));
              ...
              putget = CallInst::Create(
                getFnType, getFn, args, "", getInsertPosition(call));
              trackLLVMValue(putget);
            } else {
              Value* args[5];
              args[0] = createRnode(info, wDst, call);
              ...
              assert(getPutFn &&
                     "Missing get-put-function for global-to-global memcpy");
              putget = CallInst::Create(
                getPutFnType, getPutFn, args, "", getInsertPosition(call));
              trackLLVMValue(putget);
            }
            ...
            mset = CallInst::Create(
              memsetFnType, memsetFn, args, "", getInsertPosition(call));
            trackLLVMValue(mset);
...
void typeRemapInstruction(Instruction* I,
  ...
  unsigned i = 0;
  for (User::op_iterator op = I->op_begin(), E = I->op_end(); op != E;
       ++op, i++) {
    Value* V = *op;
    Value* oldV = V;
    Value* newV = V;
...
bool GlobalToWide::run(Module& M) {
  ...
  if (!info) {
    ...
    info->localeIdType =
      StructType::getTypeByName(M.getContext(), "struct.c_localeid_t");
    ...
    auto getFn = M.getOrInsertFunction("chpl_gen_comm_get_ctl_sym",
                                       voidTy,
                                       voidPtrTy,
                                       nodeTy,
                                       voidPtrTy,
    ...
    auto putFn = M.getOrInsertFunction("chpl_gen_comm_put_ctl_sym",
                                       voidTy,
                                       nodeTy,
                                       voidPtrTy,
                                       voidPtrTy,
    ...
    auto getPutFn = M.getOrInsertFunction("chpl_gen_comm_getput_sym",
                                          voidTy,
                                          nodeTy,
                                          voidPtrTy,
                                          nodeTy,
    ...
    auto memsetFn = M.getOrInsertFunction(
      "chpl_gen_comm_memset_sym", voidTy, nodeTy, voidPtrTy, i8Ty, sizeTy);
    ...
    for (Module::iterator next_func = M.begin(); next_func != M.end();) {
      ...
      if (starts_with(F->getName(), GLOBAL_FN_GLOBAL_ADDR) &&
          FT->getNumParams() == 1 && FT->getReturnType()->isPointerTy() &&
  ...
  // Check that a pointer in the global address space has the correct size.
  {
    ...
    llvm::Type* testGlobalTy =
      getPointerType(M.getContext(), info->globalSpace);
  ...
  for (Module::iterator next_func = M.begin(); next_func != M.end();) {
    ...
    for (Function::use_iterator UI = F->use_begin(), UE = F->use_end();
      ...
      if (auto* CB = dyn_cast<CallBase>(Old)) {
        ...
        if (InvokeInst* II = dyn_cast<InvokeInst>(Call)) {
          New = InvokeInst::Create(NF,
                                   II->getNormalDest(),
                                   II->getUnwindDest(),
                                   Args,
                                   "",
                                   getInsertPosition(Call));
          trackLLVMValue(New);
          ...
          if (cast<CallInst>(Call)->isTailCall()) NewCI->setTailCall();
        }
    ...
    if (F->begin() != F->end()) {
      ...
      if (containsGlobalPointers(info, F->getReturnType())) {
        for (Function::iterator BB = NF->begin(), E = NF->end(); BB != E;
             ++BB) {
          if (ReturnInst* RI = dyn_cast<ReturnInst>(BB->getTerminator())) {
            Instruction* New;
            New = fixer.callGlobalToWideFn(RI->getReturnValue(), RI);
            New =
              ReturnInst::Create(M.getContext(), New, getInsertPosition(RI));
            trackLLVMValue(New);
#if HAVE_LLVM_VER >= 160
            RI->eraseFromParent();
#else
...

  {
    ...
    for (Module::alias_iterator AI = M.alias_begin(), AE = M.alias_end();
      ...
      GlobalAlias* new_alias = GlobalAlias::create(getPointerType(new_type),
                                                   0, /* addr space */
                                                   ga->getLinkage(),
                                                   "",
                                                   init,
      ...
      Constant* cast_ptr =
        ConstantExpr::getPointerCast(new_alias, ga->getType());
...
bool LegacyGlobalToWidePass::runOnModule(Module& M) { return pass.run(M); }

char LegacyGlobalToWidePass::ID = 0;
static RegisterPass<LegacyGlobalToWidePass> X("global-to-wide",
                                              "GlobalToWide Pass");
...
llvm::PreservedAnalyses GlobalToWidePass::run(llvm::Module& M,
...
static bool
containsGlobalPointers(unsigned gSpace, SmallSet<Type*, 10>& set, Type* t) {
  // All primitive types do not need to change.
#if LLVM_VERSION_MAJOR >= 20
  if (t->isFloatingPointTy() || t->isLabelTy() || t->isMetadataTy() ||
      t->isVoidTy() || t->isIntegerTy())
    return false;
#else
  if (t->isFloatingPointTy() || t->isX86_MMXTy() || t->isLabelTy() ||
      t->isMetadataTy() || t->isVoidTy() || t->isIntegerTy())
    return false;
#endif
  ...
  // pointer, return true.
  bool ret = false;
  for (Type::subtype_iterator I = t->subtype_begin(), E = t->subtype_end();
       I != E;
       ++I) {
...
void populateFunctionsForGlobalType(Module* module,
                                    GlobalToWideInfo* info,
                                    Type* globalPtrTy) {
  ...
  if (!r.addrFn) {
    ...
    argTy[0] = globalPtrTy;
    r.addrFn =
      llvm::Function::Create(llvm::FunctionType::get(ptrTy, argTy, false),
  ...
  if (!r.locFn) {
    ...
    argTy[0] = globalPtrTy;
    r.locFn =
      llvm::Function::Create(llvm::FunctionType::get(locTy, argTy, false),
                             llvm::GlobalValue::ExternalLinkage,
  ...
  if (!r.nodeFn) {
    ...
    argTy[0] = globalPtrTy;
    r.nodeFn =
      llvm::Function::Create(llvm::FunctionType::get(nodeTy, argTy, false),
                             llvm::GlobalValue::ExternalLinkage,
                             GLOBAL_FN_GLOBAL_NODEID,
                             module);
...
llvm::Function* getLocFn(llvm::Module* module,
                         GlobalToWideInfo* info,
                         llvm::Type* globalPtrTy) {
  populateFunctionsForGlobalType(module, info, globalPtrTy);
  GlobalPointerInfo& r = info->gTypes[globalPtrTy];
...
void populateFunctionsForGlobalToWideType(Module* module,
                                          GlobalToWideInfo* info,
                                          Type* globalPtrTy) {
  ...
  if (!r.globalToWideFn) {
    ...
    argTy[0] = globalPtrTy;
    r.globalToWideFn =
      llvm::Function::Create(llvm::FunctionType::get(widePtrTy, argTy, false),
                             llvm::GlobalValue::ExternalLinkage,
                             GLOBAL_FN_GLOBAL_TO_WIDE,
                             module);
  ...
  if (!r.wideToGlobalFn) {
    ...
    argTy[0] = widePtrTy;
    r.wideToGlobalFn =
      llvm::Function::Create(llvm::FunctionType::get(globalPtrTy, argTy, false),
                             llvm::GlobalValue::ExternalLinkage,
...
static Type*
createWidePointerToType(Module* module, GlobalToWideInfo* i, Type* eltTy) {
...
Type* convertTypeGlobalToWide(Module* module, GlobalToWideInfo* info, Type* t) {
  ...
  if (t->isVectorTy()) {
    ...
    if (wideEltType->isStructTy() &&
        cast<StructType>(wideEltType)->getElementType(0) ==
          info->localeIdType &&
