...
BlockStmt* CForLoop::buildCForLoop(CallExpr* call,
                                   BlockStmt* body,
...
CForLoop* CForLoop::buildWithBodyFrom(ForLoop* forLoop, SymbolMap& map) {
  ...
  for_alist(expr, forLoop->body) retval->insertAtTail(expr->copy(&map, true));
...
bool CForLoop::isCForLoop() const { return true; }
...
BlockStmt* CForLoop::initBlockGet() const { return mInitClause; }

BlockStmt* CForLoop::testBlockGet() const { return mTestClause; }

BlockStmt* CForLoop::incrBlockGet() const { return mIncrClause; }
...
void CForLoop::verify() {
  ...
  if (useList != 0) INT_FATAL(this, "CForLoop::verify. useList   is not NULL");
...
void CForLoop::accept(AstVisitor* visitor) {
  if (visitor->enterCForLoop(this) == true) {
    ...
    if (initBlockGet() != 0) initBlockGet()->accept(visitor);

    if (testBlockGet() != 0) testBlockGet()->accept(visitor);

    if (incrBlockGet() != 0) incrBlockGet()->accept(visitor);
