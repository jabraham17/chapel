...
static void initializeWellKnownTypes() {
  ...
  int nAggregate =
    sizeof(sWellKnownAggregateTypes) / sizeof(sWellKnownAggregateTypes[0]);
...
AggregateType* shouldWireWellKnownType(const char* name) {
  ...
  for (int i = 0; i < nEarlyAggregate; i++) {
    ...
    if (0 == strcmp(name, wkt.name) || 0 == strcmp(name, wkt.userFacingName)) {
      return *wkt.type_;
    }
  }
...
static void multipleDefinedTypeError(Symbol* sym, const char* name) {
  USR_WARN(
    sym, "'%s' defined more than once in Chapel internal modules.", name);
}
...
static void gatherType(Symbol* sym, Type* t, const char* name) {
  ...
  for (int i = 0; i < nTypes; ++i) {
    ...
    if (name == wkt.name) {
      if (*wkt.type_ != NULL) multipleDefinedTypeError(sym, name);
      INT_ASSERT(t != NULL);
      *wkt.type_ = t;
    }
  }
  ...
  for (int i = 0; i < nAggregate; ++i) {
    ...
    if (name == wkt.name) {
      ...
      if (wkt.isClass == true && isClass(t) == false) {
        USR_FATAL_CONT(sym, "The '%s' type must be a class.", wkt.name);
      }
  ...
  forv_Vec(VarSymbol, var, gVarSymbols) {
    if (var->defPoint != nullptr &&
      ...
      if (modTag == MOD_INTERNAL || modTag == MOD_STANDARD) {
        ...
        if (t != NULL && t != dtUnknown) gatherType(var, t, var->name);
      }
    }
  }
...
std::vector<Type*> getWellKnownTypes() {
  ...
  for (int i = 0; i < nTypes; i++) {
    WellKnownType& wkt = sWellKnownTypes[i];
    if (*wkt.type_ != NULL) types.push_back(*wkt.type_);
  }
  for (int i = 0; i < nAggregate; i++) {
    WellKnownAggregateType& wkt = sWellKnownAggregateTypes[i];
    if (*wkt.type_ != NULL) types.push_back(*wkt.type_);
...
void clearGenericWellKnownTypes() {
  ...
  for (int i = 0; i < nAggregate; i++) {
    WellKnownAggregateType& wkt = sWellKnownAggregateTypes[i];
    if (*wkt.type_ != NULL && (*wkt.type_)->isGeneric()) *wkt.type_ = NULL;
  }
}
...
Type* getWellKnownTypeWithName(const char* name) {
  ...
  for (int i = 0; i < nTypes; i++) {
    WellKnownType& wkt = sWellKnownTypes[i];
    if (*wkt.type_ != NULL && wkt.name == name) return *wkt.type_;
  }
  for (int i = 0; i < nAggregate; i++) {
    WellKnownAggregateType& wkt = sWellKnownAggregateTypes[i];
    if (*wkt.type_ != NULL && wkt.name == name) return *wkt.type_;
...
static WellKnownFn sWellKnownFns[] = {
  ...
  {"chpl_taskRunningCntInc", &gChplIncRunningTask, FLAG_INC_RUNNING_TASK},

  {"chpl_taskRunningCntDec", &gChplDecRunningTask, FLAG_DEC_RUNNING_TASK},
  ...
  {"printModuleInit", &gPrintModuleInitFn, FLAG_PRINT_MODULE_INIT_FN},

  {"chpl_task_getDynamicEndCount", &gGetDynamicEndCount, FLAG_UNKNOWN},

  {"chpl_task_setDynamicEndCount", &gSetDynamicEndCount, FLAG_UNKNOWN},

  {"chpl_delete_error", &gChplDeleteError, FLAG_UNKNOWN},

  {"chpl_uncaught_error", &gChplUncaughtError, FLAG_UNKNOWN},

  {"chpl_propagate_error", &gChplPropagateError, FLAG_UNKNOWN},

  {"chpl_save_task_error", &gChplSaveTaskError, FLAG_UNKNOWN},

  {"chpl_forall_error", &gChplForallError, FLAG_UNKNOWN},

  {"atomic_fence", &gAtomicFenceFn, FLAG_UNKNOWN},

  {"chpl_after_forall_fence", &gChplAfterForallFence, FLAG_UNKNOWN},

  {"allocate_string_literals_buf", &gAllocateStringLiteralsBuf, FLAG_UNKNOWN},

  {"chpl_createStringWithLiteral", &gChplCreateStringWithLiteral, FLAG_UNKNOWN},

  {"chpl_createBytesWithLiteral", &gChplCreateBytesWithLiteral, FLAG_UNKNOWN},

  {"chpl_buildLocaleID", &gChplBuildLocaleId, FLAG_UNKNOWN},
};
...
void gatherWellKnownFns() {
  ...
  forv_Vec(FnSymbol, fn, gFnSymbols) {
    ...
    if (fn->defPoint)
      if (auto mod = fn->defPoint->getModule()) modTag = mod->modTag;
  ...
  for (int i = 0; i < nEntries; ++i) {
    ...
    if (lastMatched == NULL) {
      ...
      USR_FATAL_CONT(
        fn, "The '%s' function is missing a required flag.", wkfn.name);
    }
  }
...
std::vector<FnSymbol*> getWellKnownFunctions() {
  ...
  for (int i = 0; i < nEntries; ++i) {
    WellKnownFn& wkfn = sWellKnownFns[i];
    if (*wkfn.fn != NULL) fns.push_back(*wkfn.fn);
  }
...
void clearGenericWellKnownFunctions() {
  ...
  for (int i = 0; i < nEntries; ++i) {
    WellKnownFn& wkfn = sWellKnownFns[i];
    if (*wkfn.fn != NULL && (*wkfn.fn)->isGeneric()) *wkfn.fn = NULL;
  }
}
