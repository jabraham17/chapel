...
static QualifiedType returnInfoUnknown(CallExpr* call) {
  return QualifiedType(dtUnknown);
}

static QualifiedType returnInfoVoid(CallExpr* call) {
  return QualifiedType(dtVoid, QUAL_VAL);
}

static QualifiedType returnInfoCVoidPtr(CallExpr* call) {
  return QualifiedType(dtCVoidPtr, QUAL_VAL);
}

static QualifiedType returnInfoBool(CallExpr* call) {
  return QualifiedType(dtBool, QUAL_VAL);
}

static QualifiedType returnInfoString(CallExpr* call) {
  return QualifiedType(dtString, QUAL_VAL);
}

static QualifiedType returnInfoBytes(CallExpr* call) {
  return QualifiedType(dtBytes, QUAL_VAL);
}

static QualifiedType returnInfoStringC(CallExpr* call) {
  return QualifiedType(dtStringC, QUAL_VAL);
}

static QualifiedType returnInfoWideMake(CallExpr* call) {
  Type* t1 = call->get(1)->typeInfo();
  if (t1->symbol->hasFlag(FLAG_REF)) INT_FATAL("ref not supported here yet");

  if (wideClassMap.get(t1)) t1 = wideClassMap.get(t1);

  return QualifiedType(t1, QUAL_VAL);
}

static QualifiedType returnInfoLocaleID(CallExpr* call) {
  return QualifiedType(dtLocaleID, QUAL_VAL);
}

static QualifiedType returnInfoNodeID(CallExpr* call) {
  return QualifiedType(NODE_ID_TYPE, QUAL_VAL);
}

static QualifiedType returnInfoUInt8(CallExpr* call) {
  return QualifiedType(dtUInt[INT_SIZE_8], QUAL_VAL);
}

static QualifiedType returnInfoInt32(CallExpr* call) {
  return QualifiedType(dtInt[INT_SIZE_32], QUAL_VAL);
}

static QualifiedType returnInfoInt64(CallExpr* call) {
  return QualifiedType(dtInt[INT_SIZE_64], QUAL_VAL);
}

static QualifiedType returnInfoUInt64(CallExpr* call) {
  return QualifiedType(dtUInt[INT_SIZE_64], QUAL_VAL);
}

static QualifiedType returnInfoSizeType(CallExpr* call) {
...
// specific bit-width.
//
static QualifiedType returnInfoDefaultInt(CallExpr* call) {
...
static QualifiedType returnInfoReal32(CallExpr* call) {
  return QualifiedType(dtReal[FLOAT_SIZE_32], QUAL_VAL);
}

static QualifiedType returnInfoReal64(CallExpr* call) {
...
static QualifiedType returnInfoComplex(CallExpr* call) {
  Type* t = call->get(1)->getValType();
...
static QualifiedType returnInfoAbs(CallExpr* call) {
  Type* t = call->get(1)->getValType();
  if (t == dtComplex[COMPLEX_SIZE_64]) {
    return QualifiedType(dtReal[FLOAT_SIZE_32], QUAL_VAL);
  } else if (t == dtComplex[COMPLEX_SIZE_128]) {
...
static QualifiedType returnInfoFirst(CallExpr* call) {
  return call->get(1)->qualType();
}

static QualifiedType returnInfoSecond(CallExpr* call) {
  return call->get(2)->qualType();
}

static QualifiedType returnInfoFirstAsValue(CallExpr* call) {
  return QualifiedType(Qualifier::QUAL_CONST_VAL,
                       call->get(1)->qualType().type());
}

static QualifiedType returnInfoFirstDeref(CallExpr* call) {
...
static QualifiedType returnInfoFirstDerefNoRtti(CallExpr* call) {
  auto qt = returnInfoFirstDeref(call);
  if (qt.type()->symbol->hasFlag(FLAG_HAS_RUNTIME_TYPE)) {
    USR_FATAL(call,
              "static variables do not support types with"
              "runtime type information (such as arrays)");
  }
  return qt;
}

static QualifiedType returnInfoScalarPromotionType(CallExpr* call) {
  QualifiedType tmp = call->get(1)->qualType();
  Type* type = tmp.type()->getValType();

  if (type->scalarPromotionType) type = type->scalarPromotionType;

  return QualifiedType(type, QUAL_VAL);
}

static QualifiedType returnInfoThunkResultType(CallExpr* call) {
  QualifiedType tmp = call->get(1)->qualType();
...
static QualifiedType returnInfoStaticFieldType(CallExpr* call) {
...
static QualifiedType returnInfoCast(CallExpr* call) {
  Type* t1 = call->get(1)->typeInfo();
  ...
  if (t2->symbol->hasFlag(FLAG_WIDE_REF)) {
    if (wideRefMap.get(t1)) t1 = wideRefMap.get(t1);
  }
  INT_ASSERT(!isReferenceType(t1));
  return QualifiedType(t1, QUAL_VAL);
}

static QualifiedType returnInfoForProcTypeConverter(CallExpr* call) {
...
static QualifiedType returnInfoVal(CallExpr* call) {
...
static QualifiedType returnInfoRef(CallExpr* call) {
  Type* t = call->get(1)->getValType();
  if (t->symbol->hasFlag(FLAG_GENERIC))
    // this will result in an error later on
    return QualifiedType(t, QUAL_REF);
  if (!t->refType) INT_FATAL(call, "invalid attempt to get reference type");
  return QualifiedType(t->refType, QUAL_REF);
}

static QualifiedType returnInfoAsRef(CallExpr* call) {
  Type* t = call->get(1)->typeInfo();
...
static QualifiedType returnInfoNumericUp(CallExpr* call) {
  Type* t1 = call->get(1)->typeInfo()->getValType();
  Type* t2 = call->get(2)->typeInfo()->getValType();
  if (isIntType(t1) && isRealType(t2)) return QualifiedType(t2, QUAL_VAL);
  if (isRealType(t1) && isIntType(t2)) return QualifiedType(t1, QUAL_VAL);
  if (isIntType(t1) && isBoolType(t2)) return QualifiedType(t1, QUAL_VAL);
  if (isBoolType(t1) && isIntType(t2)) return QualifiedType(t2, QUAL_VAL);
  return QualifiedType(t1, QUAL_VAL);
}

static QualifiedType returnInfoArrayIndexValue(CallExpr* call) {
  Type* type = call->get(1)->getValType();
...
static QualifiedType returnInfoArrayIndex(CallExpr* call) {
  QualifiedType tmp = returnInfoArrayIndexValue(call);
  auto refType = tmp.type()->refType;
  if (!refType) INT_FATAL(call, "invalid attempt to get reference type");
  return QualifiedType(refType, QUAL_REF);
}

static QualifiedType returnInfoGetMember(CallExpr* call) {
  AggregateType* ct = toAggregateType(call->get(1)->typeInfo());
  if (!ct) INT_FATAL(call, "bad member primitive");
  if (ct->symbol->hasFlag(FLAG_REF)) ct = toAggregateType(ct->getValType());
  SymExpr* sym = toSymExpr(call->get(2));
  if (!sym) INT_FATAL(call, "bad member primitive");
  VarSymbol* var = toVarSymbol(sym->symbol());
  if (!var) INT_FATAL(call, "bad member primitive");
...
static QualifiedType returnInfoGetTupleMember(CallExpr* call) {
...
static QualifiedType returnInfoGetTupleMemberRef(CallExpr* call) {
...
static QualifiedType returnInfoGetMemberRef(CallExpr* call) {
...
static QualifiedType returnInfoEndCount(CallExpr* call) {
...
static QualifiedType returnInfoError(CallExpr* call) {
...
static QualifiedType returnInfoVirtualMethodCall(CallExpr* call) {
...
static QualifiedType returnInfoCoerce(CallExpr* call) {
  ...
  if (t->symbol->hasFlag(FLAG_GENERIC)) {
    ...
    t = getInstantiationType(actualOne->getValType(),
                             actualOne->symbol(),
                             t,
                             actualTwo->symbol(),
                             call);
    if (t == nullptr)
      USR_FATAL(call,
                "could not coerce a value of type '%s' to type '%s'",
                actualOne->getValType()->symbol->name,
                actualTwo->symbol()->name);
  }
...
static QualifiedType returnInfoToUnmanaged(CallExpr* call) {
  Type* t = call->get(1)->getValType();
...
static QualifiedType returnInfoToBorrowed(CallExpr* call) {
  Type* t = call->get(1)->getValType();
  ...
  if (AggregateType* at = toAggregateType(canonicalClassType(t)))
    if (isClass(at)) t = at->getDecoratedClass(decorator);

  return QualifiedType(t, QUAL_VAL);
}

static QualifiedType returnInfoToUndecorated(CallExpr* call) {
  Type* t = call->get(1)->getValType();
...
static QualifiedType returnInfoToNilable(CallExpr* call) {
  Type* t = call->get(1)->getValType();
...
static QualifiedType returnInfoToNonNilable(CallExpr* call) {
  Type* t = call->get(1)->getValType();
...
static QualifiedType returnInfoRuntimeTypeField(CallExpr* call) {
  bool isType = false;
...
PrimitiveOp::PrimitiveOp(PrimitiveTag atag,
  ...
  : tag(atag), name(aname), returnInfo(areturnInfo), isEssential(false),
    passLineno(false) {
...
#define PRIMITIVE(macroTag, macroName)                                   \
  case PRIM_##macroTag: INT_ASSERT(0 == strcmp(name, macroName)); break;
...
static void prim_def(PrimitiveTag tag,
                     const char* name,
                     QualifiedType (*returnInfo)(CallExpr*),
                     bool isEssential = false,
                     bool passLineno = false) {
...
void initPrimitive() {
  ...
  prim_def(
    PRIM_THUNK_RESULT_TYPE, "thunk result type", returnInfoThunkResultType);
  ...
  // dst, optional type
  prim_def(
    PRIM_INIT_VAR_SPLIT_DECL, "init var split decl", returnInfoVoid, false);
  ...
  prim_def(
    PRIM_SPLIT_INIT_UPDATE_TYPE, "split init update type", returnInfoVoid);
  ...
  prim_def(
    PRIM_GET_DYNAMIC_END_COUNT, "get dynamic end count", returnInfoEndCount);
  prim_def(
    PRIM_SET_DYNAMIC_END_COUNT, "set dynamic end count", returnInfoVoid, true);

  // this requires sizes in all 3D to be specified. For 2D launches, 1 can be
  // passed as one or more of these arguments.
  prim_def(PRIM_GPU_KERNEL_LAUNCH,
           "gpu kernel launch",
           returnInfoVirtualMethodCall,
           true);
  ...
  prim_def(PRIM_GPU_INIT_KERNEL_CFG,
           "gpu init kernel cfg",
           returnInfoCVoidPtr,
           true,
           true);
  prim_def(PRIM_GPU_INIT_KERNEL_CFG_3D,
           "gpu init kernel cfg 3d",
           returnInfoCVoidPtr,
           true,
           true);
  prim_def(
    PRIM_GPU_DEINIT_KERNEL_CFG, "gpu deinit kernel cfg", returnInfoVoid, true);
  ...
  prim_def(
    PRIM_GPU_ATTRIBUTE_BLOCK, "gpu attribute block", returnInfoVoid, true);
  ...
  // The marker prevents the block from being flattened by other passes.
  prim_def(
    PRIM_GPU_PRIMITIVE_BLOCK, "gpu primitive block", returnInfoVoid, true);
  ...
  prim_def(PRIM_GPU_SET_ITERS_PER_THREAD,
           "gpu set itersPerThread",
           returnInfoVoid,
           true);

  // Generates call that produces runtime error when not run by a GPU
  prim_def(
    PRIM_ASSERT_ON_GPU, "chpl_assert_on_gpu", returnInfoVoid, true, true);
  prim_def(PRIM_ASSERT_GPU_ELIGIBLE,
           "assert gpu eligible",
           returnInfoVoid,
           true,
           true);
  ...
  prim_def(
    PRIM_SIZEOF_DDATA_ELEMENT, "sizeof_ddata_element", returnInfoSizeType);
  ...
  prim_def(PRIM_IS_INSTANTIATION_ALLOW_VALUES,
           "is_instantiation_allow_values",
           returnInfoBool);
  ...
  prim_def(PRIM_STATIC_FUNCTION_VAR_VALIDATE_TYPE,
           "static function validate type",
           returnInfoFirstDerefNoRtti);
  prim_def(PRIM_STATIC_FUNCTION_VAR_WRAPPER,
           "static function var wrapper",
           returnInfoVoid);
  ...
  // There might be uninitialized memory if the run-time type is used.
  prim_def(PRIM_SCALAR_PROMOTION_TYPE,
           "scalar promotion type",
           returnInfoScalarPromotionType);
  ...
  prim_def(
    PRIM_STATIC_FIELD_TYPE, "static field type", returnInfoStaticFieldType);
  ...
  prim_def(
    PRIM_REFERENCED_MODULES_LIST, "referenced modules list", returnInfoVoid);
  ...
  prim_def(PRIM_CHPL_COMM_ARRAY_GET,
           "chpl_comm_array_get",
           returnInfoVoid,
           true,
           true);
  prim_def(PRIM_CHPL_COMM_ARRAY_PUT,
           "chpl_comm_array_put",
           returnInfoVoid,
           true,
           true);
  prim_def(PRIM_CHPL_COMM_REMOTE_PREFETCH,
  ...
  prim_def(
    PRIM_CHPL_COMM_GET_STRD, "chpl_comm_get_strd", returnInfoVoid, true, true);
  prim_def(
    PRIM_CHPL_COMM_PUT_STRD, "chpl_comm_put_strd", returnInfoVoid, true, true);

  prim_def(
    PRIM_ARRAY_SHIFT_BASE_POINTER, "shift_base_pointer", returnInfoVoid, true);
  ...
  prim_def(
    PRIM_MAYBE_LOCAL_ARR_ELEM, "may be local array element", returnInfoUnknown);
  prim_def(PRIM_MAYBE_AGGREGATE_ASSIGN,
           "may be aggregated assignment",
           returnInfoUnknown);
  ...
  prim_def(
    PRIM_GET_REQUESTED_SUBLOC, "chpl_task_getRequestedSubloc", returnInfoInt64);

  prim_def(PRIM_REGISTER_GLOBAL_VAR,
           "_register_global_var",
           returnInfoVoid,
           true,
           true);
  prim_def(PRIM_BROADCAST_GLOBAL_VARS,
           "_broadcast_global_vars",
           returnInfoVoid,
           true,
           true);
  ...
  prim_def(
    PRIM_STRING_LENGTH_BYTES, "string_length_bytes", returnInfoDefaultInt);
  prim_def(PRIM_STRING_LENGTH_CODEPOINTS,
           "string_length_codepoints",
           returnInfoDefaultInt);
  ...
  prim_def(PRIM_CAST_TO_VOID_STAR,
           "cast_to_void_star",
           returnInfoCVoidPtr,
           true,
           false);
  ...
  prim_def(PRIM_CAST_TO_TYPE,
           "cast_to_type",
           returnInfoSecondActualTypeSymbol,
           true,
           false);
  ...
  prim_def(
    PRIM_GET_USER_LINE, "_get_user_line", returnInfoDefaultInt, true, true);
  ...
  prim_def(
    PRIM_GET_SVEC_MEMBER, "get svec member", returnInfoGetTupleMemberRef);
  prim_def(PRIM_GET_SVEC_MEMBER_VALUE,
           "get svec member value",
           returnInfoGetTupleMember,
           false);

  prim_def(PRIM_VIRTUAL_METHOD_CALL,
           "virtual method call",
           returnInfoVirtualMethodCall,
           true);
  ...
  prim_def(PRIM_ITERATOR_RECORD_FIELD_VALUE_BY_FORMAL,
           "iterator record field value by formal",
           returnInfoIteratorRecordFieldValueByFormal);
  prim_def(PRIM_ITERATOR_RECORD_SET_SHAPE,
           "iterator record set shape",
           returnInfoVoid);
  ...
  prim_def(PRIM_IS_NON_NILABLE_CLASS_TYPE,
           "is non nilable class type",
           returnInfoBool);
  ...
  prim_def(
    PRIM_IS_BORROWED_CLASS_TYPE, "is borrowed class type", returnInfoBool);
  ...
  prim_def(
    PRIM_IS_CONST_ASSIGNABLE, "is const assignable type", returnInfoBool);
  ...
  prim_def(PRIM_TO_UNMANAGED_CLASS_CHECKED,
           "to unmanaged class from unknown",
           returnInfoToUnmanaged,
           false,
           false);
  // return the borrowed class variant, error if decorated
  prim_def(PRIM_TO_BORROWED_CLASS_CHECKED,
           "to borrowed class from unknown",
           returnInfoToBorrowed,
           false,
           false);
  // return the nilable class variant, error if argument is not a type
  prim_def(PRIM_TO_NILABLE_CLASS_CHECKED,
           "to nilable class from type",
           returnInfoToNilable,
           false,
           false);

  // These return the (non-nil) class variant
  prim_def(PRIM_TO_UNMANAGED_CLASS,
           "to unmanaged class",
           returnInfoToUnmanaged,
           false,
           false);
  // borrowed class type currently == canonical class type
  prim_def(PRIM_TO_BORROWED_CLASS,
           "to borrowed class",
           returnInfoToBorrowed,
           false,
           false);
  // return the undecorated class type
  prim_def(PRIM_TO_UNDECORATED_CLASS,
           "to undecorated class",
           returnInfoToUndecorated,
           false,
           false);
  // Returns the nilable class type
  prim_def(PRIM_TO_NILABLE_CLASS,
           "to nilable class",
           returnInfoToNilable,
           false,
           false);
  prim_def(PRIM_TO_NON_NILABLE_CLASS,
           "to non nilable class",
           returnInfoToNonNilable,
           false,
           false);

  prim_def(PRIM_SET_ALIASING_ARRAY_ON_TYPE,
           "set aliasing array on type",
           returnInfoVoid,
           false,
           false);

  prim_def(PRIM_NEEDS_AUTO_DESTROY,
           "needs auto destroy",
           returnInfoBool,
           false,
           false);
  ...
  prim_def(
    PRIM_END_OF_STATEMENT, "end of statement", returnInfoVoid, false, false);

  prim_def(PRIM_AUTO_DESTROY_RUNTIME_TYPE,
           "auto destroy runtime type",
           returnInfoVoid,
           false,
           false);
  ...
  prim_def(PRIM_GET_RUNTIME_TYPE_FIELD,
           "get runtime type field",
           returnInfoRuntimeTypeField,
           false,
           false);
  ...
  prim_def(
    PRIM_INVARIANT_START, "invariant start", returnInfoVoid, false, false);
  ...
  prim_def(PRIM_COPIES_NO_ALIAS_SET,
           "copies no alias set",
           returnInfoUnknown,
           false,
           false);
  ...
  prim_def(
    PRIM_OPTIMIZATION_INFO, "optimization info", returnInfoVoid, true, false);
  ...
  prim_def(
    PRIM_REF_DESERIALIZE, "deserialize for ref fields", returnInfoCVoidPtr);
  ...
  prim_def(PRIM_CHECK_CONST_ARG_HASH,
           "check hashes of const arguments",
           returnInfoVoid,
           true,
           true);
  ...
  prim_def(PRIM_TASK_PRIVATE_SVAR_CAPTURE,
           "task private svar capture",
           returnInfoUnknown);
}
...
VarSymbol* newMemDesc(const char* str) {
  ...
  if (VarSymbol* v = memDescsMap.get(s)) return v;
...
VarSymbol* newMemDesc(Type* type) {
  if (VarSymbol* v = memDescsNodeMap.get(type->id)) return v;
  VarSymbol* memDescVar = new_IntSymbol(memDescInt++, INT_SIZE_16);
...
void registerPrimitiveCodegen(PrimitiveTag tag,
                              void (*fn)(CallExpr*, GenRet&)) {
  for (int i = 0; i < NUM_KNOWN_PRIMS; i++) {
