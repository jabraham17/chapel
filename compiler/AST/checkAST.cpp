  ...
  for_alive_in_Vec(CallExpr, call, gCallExprs) {
    ...
    switch (call->primitive->tag) {
      ...
      case PRIM_WARNING:
        if (resolved)
          INT_FATAL(
            "Primitive should not appear after resolution is complete.");
        break;
      ...
      case PRIM_SET_MEMBER:
        if (resolved) {
          ...
          for_fields(field, ct) {
            if (0 == strcmp(field->name, getField->name)) name_match = field;
          }
...
void checkReturnTypesHaveRefTypes() {
  for_alive_in_Vec(FnSymbol, fn, gFnSymbols) {
    ...
    if (retType->symbol->hasFlag(FLAG_REF)) continue;
