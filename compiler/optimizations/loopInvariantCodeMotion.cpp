...
class Loop {
 ...
 public:
  ...
  LoopStmt* getLoopAST() const { return loopAST; }
  ...
  bool contains(int i) { return bitBlocks->test(i); }

  //check if a block is in the loop
  bool contains(BasicBlock* block) { return bitBlocks->test(block->id); }

  //get the header block
  BasicBlock* getHeader() { return header; }

  //get the actual blocks in the loop
  std::vector<BasicBlock*>* getBlocks() { return loopBlocks; }

  //get the bitvector that represents the blocks in the loop
  BitVec* getBitBlocks() { return bitBlocks; }
  ...
  unsigned size() { return loopBlocks->size(); }
...
void collectNaturalLoops(std::vector<Loop*>& loops,
                         BasicBlocks& basicBlocks,
                         BasicBlock* entryBlock,
                         std::vector<BitVec*>& dominators);
void collectNaturalLoopForEdge(Loop* loop,
                               BasicBlock* header,
                               BasicBlock* tail);
...
//
//
static Symbol* rhsAlias(CallExpr* call) {
 ...
 */
static void addDefOrUse(symToVecSymExprMap& localDefOrUseMap,
                        Symbol* var,
                        SymExpr* defOrUse) {
 ...
 */
static void buildLocalDefUseMaps(Loop* loop,
        ...
        if (symExpr->parentSymbol) {
          ...
          if (CallExpr* callExpr = toCallExpr(symExpr->parentExpr)) {
            if (callExpr->isResolved() ||
                callExpr->isPrimitive(PRIM_VIRTUAL_METHOD_CALL)) {
              addDefOrUse(localDefMap, symExpr->symbol(), symExpr);
              Type* type = symExpr->symbol()->type->symbol->type;
              if (AggregateType* curClass = toAggregateType(type)) {
 ...
 */
static void freeLocalDefUseMaps(symToVecSymExprMap& localDefMap,
                                symToVecSymExprMap& localUseMap) {
 ...
 */
static bool allOperandsAreLoopInvariant(
  Expr* expr,
  std::set<SymExpr*>& loopInvariants,
  ...
  if (CallExpr* callExpr = toCallExpr(expr)) {
    if (callExpr->primitive && isLoopInvariantPrimitive(callExpr->primitive)) {
      if (callExpr->isPrimitive(PRIM_MOVE) ||
        ...
        return allOperandsAreLoopInvariant(callExpr->get(2),
                                           loopInvariants,
        ...
        for_alist(arg, callExpr->argList) {
          if (allOperandsAreLoopInvariant(arg,
                                          loopInvariants,
                                          loopInvariantInstructions,
                                          loop,
                                          actualDefs) == false) {
...
static bool computeAliases(FnSymbol* fn,
...
#ifdef detailedTiming
      FILE* tooManyAliasesFile =
        fopen(astr(CHPL_HOME, "/LICMaliases.txt"), "a");
      fprintf(tooManyAliasesFile,
              "Skipping fn %s %d of module %s %d "
 ...
 */
static void
computeLoopInvariants(std::vector<SymExpr*>& loopInvariants,
  ...
  for_vector(SymExpr, symExpr, loopSymExprs) {
          ...
          for_vector(SymExpr, aliasSymExpr, *localDefMap[symbol]) {
            if (CallExpr* call = toCallExpr(aliasSymExpr->parentExpr)) {
              if (symExpr->symbol() == rhsAlias(call)) {
                //do nothing
              } else if (aliases[symExpr->symbol()].count(rhsAlias(call)) ==
                         0) {
    ...
    if (isLcnSymbol(symExpr->symbol())) {
      ...
      if (AggregateType* ct =
            toAggregateType(symExpr->symbol()->type->symbol->type)) {
    ...
    if (!declaredWithConstIntent && (!isGpuBound || !isArray)) {
      if (isArgSymbol(symExpr->symbol()) &&
          symExpr->getValType()->symbol->hasFlag(FLAG_ITERATOR_CLASS) ==
            false) {
  ...
  do {
    ...
    for_vector(SymExpr, symExpr2, loopSymExprs) {
      ...
      if (loopInvariantInstructions.count(symExpr2) == 1 ||
          loopInvariantOperands.count(symExpr2) == 1) {
        continue;
      }
            ...
            if (CallExpr* callExpr = toCallExpr(symExpr2->parentExpr)) {
              if (callExpr->isPrimitive(PRIM_MOVE) ||
                ...
                if (callExpr->get(1) == symExpr2) {
                  ...
                  bool loopInvarOps =
                    allOperandsAreLoopInvariant(callExpr,
                                                loopInvariantOperands,
                                                loopInvariantInstructions,
                                                loop,
                                                actualDefs);
  ...
  for_set(SymExpr, loopInvariant, loopInvariantInstructions) {
    printf("Symbol %s with id %d is a hoistable loop invariant\n",
           loopInvariant->symbol()->name,
           loopInvariant->symbol()->id);
  }
 ...
 *
 */
static bool defDominatesAllUses(Loop* loop,
                                SymExpr* def,
                                std::vector<BitVec*>& dominators,
                                std::map<SymExpr*, int>& localMap,
                                symToVecSymExprMap& localUseMap) {
 ...
 *
 */
static bool defDominatesAllExits(Loop* loop,
                                 SymExpr* def,
                                 std::vector<BitVec*>& dominators,
                                 std::map<SymExpr*, int>& localMap) {
  if (def->symbol()->defPoint != nullptr &&
      LoopStmt::findEnclosingLoop(def->symbol()->defPoint) ==
        loop->getLoopAST()) {
...
static void licmFn(FnSymbol* fn) {
  ...
  for_vector(Loop, curLoop, loops) {
    ...
    computeLoopInvariants(
      loopInvariants, defsInLoop, curLoop, localDefMap, aliases);
    ...
    for_vector(SymExpr, symExpr, loopInvariants) {
      if (CallExpr* call = toCallExpr(symExpr->parentExpr)) {
        if (defDominatesAllUses(
              curLoop, symExpr, dominators, localMap, localUseMap)) {
...
static void loopInvariantCodeMotionImpl(void) {
  ...
  forv_Vec(FnSymbol, fn, gFnSymbols) { licmFn(fn); }
  ...
  fprintf(timingFile,
          "For compilation of %s:                         \n",
          compileCommand);
  fprintf(timingFile,
          "Spent %2.3f seconds building basic blocks      \n",
          buildBBTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds computing dominators       \n",
          computeDominatorTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds collecting natural loops   \n",
          collectNaturalLoopsTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds building local def maps    \n",
          buildLocalDefMapsTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds on can perform code motion \n",
          canPerformCodeMotionTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds computing loop invariants  \n",
          computeLoopInvariantsTimer.elapsedSecs());

  double estimateOverall = buildBBTimer.elapsedSecs() +
                           computeDominatorTimer.elapsedSecs() +
                           collectNaturalLoopsTimer.elapsedSecs() +
                           buildLocalDefMapsTimer.elapsedSecs() +
                           canPerformCodeMotionTimer.elapsedSecs() +
  ...
  fprintf(timingFile,
          "Spent %2.3f seconds loop hoisting on %ld loops \n",
          overallTimer.elapsedSecs(),
          numLoops);
  fprintf(timingFile,
          "      %2.3f seconds unaccounted for            \n",
          overallTimer.elapsedSecs() - estimateOverall);

  double estimateInvariant = allOperandsAreLoopInvariantTimer.elapsedSecs() +
  ...
  fprintf(timingFile,
          "Spent %2.3f seconds collecting sym/def Expr    \n",
          collectSymExprAndDefTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds computing aliases          \n",
          computeAliasTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds calculating actual defs    \n",
          calculateActualDefsTimer.elapsedSecs());
  fprintf(timingFile,
          "Spent %2.3f seconds in allOperandsAreInvariant \n",
