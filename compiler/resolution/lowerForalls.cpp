...
ExpandVisitor::ExpandVisitor(ForallStmt* fs, SymbolMap& map)
  : forall(fs), svar2clonevar(map), delayedRemoval() {}

// constructor for a nested situation
ExpandVisitor::ExpandVisitor(ExpandVisitor* parentEV, SymbolMap& map)
  : forall(parentEV->forall), svar2clonevar(map), delayedRemoval() {}

ExpandVisitor::~ExpandVisitor() {
  for_vector(Expr, expr, delayedRemoval) expr->remove();
}
...
static void addDefAndMap(Expr* aInit,
                         SymbolMap& map,
...
static void
addCloneOfInitBlock(Expr* aInit, SymbolMap& map, ShadowVarSymbol* svar) {
  //
...
static void
addCloneOfDeinitBlock(Expr* aFini, SymbolMap& map, ShadowVarSymbol* svar) {
...
static void expandYield(ExpandVisitor* EV, CallExpr* yieldCall) {
  ...
  addIteratorBreakBlocksInline(
    EV->forall->loopBody(), NULL, bodyClone, yieldCall, &EV->delayedRemoval);
...
//
static void addFormalTempSIifNeeded(FnSymbol* cloneTaskFn,
                                    Expr* aInit,
                                    SymbolMap& map,
                                    ShadowVarSymbol* SI) {
  ...
  if (!formalRequiresTemp(eFormal, cloneTaskFn)) return; // not that case
...
static ArgSymbol*
newExtraFormal(ShadowVarSymbol* svar, int ix, Symbol* eActual, bool nested) {
  ...
  if (efInt & INTENT_FLAG_REF) {
    ...
    if (eActual->isConstValWillNotChange()) addFlagImm = true;
  }
...
static Symbol* eActualOrRef(Expr* ref, ShadowVarSymbol* svar, Symbol* eActual) {
  if (!svar->isRef() || eActual->isRef()) return eActual;
...
static void addArgAndMap(FnSymbol* cloneTaskFn,
                         CallExpr* callToTFn,
                         int numOrigActuals,
                         SymbolMap& iMap,
                         SymbolMap& map,
...
static void expandShadowVarTaskFn(FnSymbol* cloneTaskFn,
                                  CallExpr* callToTFn,
                                  Expr* aInit,
                                  Expr* aFini,
                                  int numOrigAct,
  ...
  switch (svar->intent) {
    case TFI_IN:
    case TFI_CONST_IN:
      addArgAndMap(cloneTaskFn, callToTFn, numOrigAct, iMap, map, svar, ix);
    ...
    case TFI_REDUCE: {
      ...
      */
      ...
      addArgAndMap(cloneTaskFn, callToTFn, numOrigAct, iMap, map, RP, ix, PRP);
    ...
    case TFI_REDUCE_PARENT_AS:
    case TFI_REDUCE_PARENT_OP: break;
...
static void
expandTaskFn(ExpandVisitor* EV, CallExpr* callToTFn, FnSymbol* taskFn) {
  ...
  if (!preserveInlinedLineNumbers) reset_ast_loc(cloneTaskFn, callToTFn);
  ...
  aInit = new CallExpr("aInit");
  cloneTaskFn->insertAtHead(aInit);
  aFini = new CallExpr("aFini");
  cloneTaskFn->insertIntoEpilogue(aFini);
  ...
  for_shadow_vars(svar, temp, EV->forall) expandShadowVarTaskFn(cloneTaskFn,
                                                                callToTFn,
                                                                aInit,
                                                                aFini,
                                                                numOrigActuals,
  ...
  if (addErrorArgToCall) addDummyErrorArgumentToCall(callToTFn);
...
static void expandShadowVarForall(ForallStmt* fs,
  ...
  switch (newSV->intent) {
    ...
    case TFI_REF:
    case TFI_CONST_REF:
    case TFI_REDUCE: newOvar = srcSV; break; // the current var just before 'fs'

    case TFI_IN_PARENT:
      newOvar = srcSV->INforParentvar();
      break; // the current var for SI

    case TFI_REDUCE_PARENT_AS:
      newOvar = srcSV->AccumStateForParentAS();
      break; // current AS var

    case TFI_REDUCE_PARENT_OP:
      newOvar = srcSV->ReduceOpForParentRP();
      break; // current RP var

    case TFI_REDUCE_OP:
    case TFI_TASK_PRIVATE: break; // no outer var
...
static void expandForall(ExpandVisitor* EV, ForallStmt* fs) {
  ...
  for_shadow_vars(srcSV, temp, pfs) expandShadowVarForall(fs, iMap, map, srcSV);
...
static void expandShadowVarTopLevel(Expr* aInit,
                                    Expr* aFini,
                                    SymbolMap& map,
                                    FnSymbol* parIterFn,
                                    ShadowVarSymbol* svar) {
...
static bool shouldGoFirst(ShadowVarSymbol* sv) {
  switch (sv->intent) {
    ...
    case TFI_CONST_IN:
    case TFI_REF:
    case TFI_CONST_REF: return true;
...
static void warnRecursiveIterNonrefSVar(ForallStmt* fs,
                                        FnSymbol* parIterFn,
                                        ShadowVarSymbol* svar,
                                        bool& gotNonRefs,
                                        bool& implWarn) {
  if (gotNonRefs == false) {
    ...
    USR_PRINT(fs,
              "such loops are currently implemented only if\n"
  ...
  if (!svar->isCompilerAdded()) {
    if (implWarn == false) USR_PRINT(fs, "a shadow variable may be implicit");
    implWarn = true;
    USR_PRINT(svar,
              "'%s' shadow variable has %s",
              svar->name,
...
static void handleRecursiveIter(ForallStmt* fs,
  ...
  PARBlock->insertAtTail(
    new CallExpr(PRIM_MOVE, parIter, callGetIter->remove()));

  ForLoop* PARBody = new ForLoop(parIdx,
                                 parIter,
                                 NULL,
                                 /* zippered */ false,
                                 /*forall*/ true,
...
static bool isProperIterator(FnSymbol* fn) { return fn->isIterator(); }
...
static void removeAutoDestroyCallIfPresent(Symbol* currSym) {
  for_SymbolSymExprs(curSE, currSym) if (
    CallExpr* curCall = toCallExpr(
      curSE
        ->parentExpr)) if (FnSymbol* curCallFn =
                             curCall
                               ->resolvedFunction()) if (curCallFn
                                                           ->hasFlag(
...
*/
static Symbol*
inlineRetArgFunction(CallExpr* defCall, FnSymbol* defFn, Symbol* currSym) {
  BlockStmt* defBody =
    copyFnBodyForInlining(defCall, defFn, defCall->getStmtExpr());
...
static CallExpr* stripReturnScaffolding(BlockStmt* block, Symbol* currSym) {
  ...
  while (true) {
        ...
        if (defMove->isPrimitive(PRIM_MOVE) ||
          ...
          if (CallExpr* srcCall = toCallExpr(defSrc)) {
            if (srcCall->resolvedFunction()->hasFlag(FLAG_AUTO_COPY_FN)) {
              ...
              removeAutoDestroyCallIfPresent(currSym);
              INT_ASSERT(currSym->firstSymExpr() ==
                         NULL); // no other refs to it
              continue;
            }
    ...
    USR_FATAL(currSym,
              "only simple control flow is allowed in a procedure that returns "
              "an iterator for use in a forall loop");
    break;
  }
...
static void lowerOneForallStmt(ForallStmt* fs) {
  ...
  if (isVirtualIterator(parIterFn)) {
    USR_FATAL_CONT(
      fs, "virtual parallel iterators are not yet supported (see issue #6998)");
    return;
  }
...
void lowerForallStmtsInline() {
  forv_expanding_Vec(ForallStmt, fs, gForallStmts) if (fs->inTree())
