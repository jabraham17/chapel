...
enum TryTag { TRY_TAG_NONE, TRY_TAG_IN_TRY, TRY_TAG_IN_TRYBANG };
...
CallExpr* callChplHereAllocWithAllocator(Type* type,
                                         Expr* allocator,
                                         VarSymbol* md = NULL);

void insertChplHereAlloc(
  Expr* call, bool insertAfter, Symbol* sym, Type* t, VarSymbol* md = NULL);
...
static inline bool isEndOfStatementMarker(Expr* e) {
  if (CallExpr* call = toCallExpr(e))
    if (call->isPrimitive(PRIM_END_OF_STATEMENT)) return true;
...
inline FnSymbol* CallExpr::theFnSymbol() const { return resolvedFunction(); }

inline bool CallExpr::isResolved() const { return resolvedFunction() != NULL; }

inline bool CallExpr::isEmpty() const {
  return primitive == NULL && baseExpr == NULL;
}

inline bool CallExpr::isPrimitive() const { return primitive != NULL; }
...
inline int CallExpr::numActuals() const { return argList.length; }

inline Expr* CallExpr::get(int index) const { return argList.get(index); }
