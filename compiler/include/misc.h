...
#define USR_WARN                                                            \
  setupError(TOSTRING(COMPILER_SUBDIR), __FILE__, __LINE__, 4), handleError

#define USR_PRINT                                                           \
  setupError(TOSTRING(COMPILER_SUBDIR), __FILE__, __LINE__, 5), handleError
...
#define INT_ASSERT(...)                                             \
  SELECT_ASSERT(__VA_ARGS__, INT_ASSERT2, INT_ASSERT1)(__VA_ARGS__)
#define INT_ASSERT1(x)                      \
  do {                                      \
    if (!(x)) INT_FATAL("assertion error"); \
  } while (0)
#define INT_ASSERT2(s, x)                        \
  do {                                           \
    if (!(x)) INT_FATAL((s), "assertion error"); \
  } while (0)
...
void handleError(astlocT astloc, const char* fmt, ...)
  __attribute__((format(printf, 2, 3)));
void handleError(chpl::Location, const char* fmt, ...)
  __attribute__((format(printf, 2, 3)));
