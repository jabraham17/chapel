...
static PassInfo sPassList[] = {
  ...
  RUN(
    replaceArrayAccessesWithRefTemps), // replace multiple array access calls with reference temps
...
void runPasses(PhaseTracker& tracker) {
  ...
  for (size_t i = 0; i < passListSize; i++) {
    ...
    if (fDriverMakeBinaryPhase &&
        strcmp(sPassList[i].name, "makeBinary") != 0) {
      continue;
    }
...
static void runPass(PhaseTracker& tracker, size_t passIndex) {
  ...
  if (fPrintStatistics[0] != '\0' && passIndex > 0) printStatistics("clean");
  ...
  if (fPrintStatistics[0] != '\0') printStatistics(info->name);
