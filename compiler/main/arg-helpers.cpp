...
void arg_fatalError(const char* msg) { INT_FATAL("%s", msg); }

void arg_fatalUserError(const char* msg) { USR_FATAL("%s", msg); }

const char* arg_programLoc(const char* argv0, void* mainAddr) {
  return findProgramPath(argv0);
}

void arg_fatalErrorCont(const char* msg) { USR_FATAL_CONT("%s", msg); }
