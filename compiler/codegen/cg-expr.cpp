...
static GenRet
codegenCallExpr(const char* fnName, GenRet a1, GenRet a2, GenRet a3);
...
static void
codegenCall(const char* fnName, GenRet a1, GenRet a2, GenRet a3, GenRet a4);
...
static bool shouldGenerateAsIfCallingDirectly(SymExpr* se, FnSymbol* fn) {
  ...
  if (auto call = toCallExpr(se->parentExpr)) {
    ...
    bool isCastedToPtr =
      call->isPrimitive(PRIM_CAST_TO_TYPE) || call->isPrimitive(PRIM_CAST);
  ...
  if (info->cfile) {
    ...
    if (toLabelSymbol(sym)) {
      ...
      if (!(blockLabel = info->lvt->getBlock(sym->cname))) {
        blockLabel =
          llvm::BasicBlock::Create(info->module->getContext(), sym->cname);
...
createVarLLVM(llvm::Type* type, const char* name, int alignment) {
  llvm::AllocaInst* result =
    createVarLLVM(type, (Type*)nullptr, (Symbol*)nullptr, name);
...
llvm::Value* convertValueToType(llvm::Value* value,
  ...
  val = convertValueToType(
    irBuilder, layout, ctx, value, newType, &alloca, isSigned, force);
...
static PromotedPair convertValuesToLarger(llvm::Value* value1,
                                          llvm::Value* value2,
                                          bool isSigned1 = false,
                                          bool isSigned2 = false) {
  GenInfo* info = gGenInfo;
  return convertValuesToLarger(
    info->irBuilder, value1, value2, isSigned1, isSigned2);
}
...
static llvm::Value* createInBoundsGEP(llvm::Type* gepType,
  ...
  llvm::Value* gep =
    info->irBuilder->CreateInBoundsGEP(gepType, ptr, idxList, name);
  trackLLVMValue(gep);
  return gep;
}
...
// all the cases.
static GenRet
codegenWideAddr(GenRet locale, GenRet raddr, Type* wideType = NULL) {
  GenRet ret;
  GenInfo* info = gGenInfo;
  ...
  if (raddr.chplType && !wideType) {
    ...
    if (raddr.isLVPtr == GEN_VAL) {
      ...
      INT_ASSERT(raddr.chplType == dtNil || isClass(raddr.chplType) ||
  ...
  if (!fLLVMWideOpt) {
    ...
    if (info->cfile) {
      ...
      llvm::Value* adr =
        info->irBuilder->CreateStructGEP(gepTy, ret.val, WIDE_GEP_ADDR);
      llvm::Value* loc =
        info->irBuilder->CreateStructGEP(gepTy, ret.val, WIDE_GEP_LOC);
      trackLLVMValue(adr);
      trackLLVMValue(loc);
    ...
    llvm::Function* fn =
      getMakeFn(info->module, &info->globalToWideInfo, addrType);
    INT_ASSERT(fn);
...
static GenRet
codegenWideAddrWithAddr(GenRet base, GenRet newAddr, Type* wideType = NULL) {
...
static llvm::Type* computePointerElementType(llvm::Value* ptr, Type* chplType) {
...
static void codegenInvariantStart(llvm::Type* valType, llvm::Value* addr) {
  GenInfo* info = gGenInfo;
...
codegenStoreLLVM(llvm::Value* val,
                 llvm::Value* ptr,
                 Type* valType = NULL,
  ...
  if (noalias) ret->setMetadata(llvm::LLVMContext::MD_noalias, noalias);
...
static llvm::StoreInst*
codegenStoreLLVM(GenRet val, GenRet ptr, Type* valType = NULL) {
  if (val.chplType && !valType) valType = val.chplType;
  if (ptr.chplType && !valType) {
    if (ptr.isLVPtr)
      valType = ptr.chplType;
    else
      valType = ptr.chplType->getValType();
  }
  ...
  return codegenStoreLLVM(val.val,
                          ptr.val,
                          valType,
                          ptr.surroundingStruct,
                          ptr.fieldOffset,
                          ptr.fieldTbaaTypeDescriptor,
                          ptr.aliasScope,
                          ptr.noalias,
...
static llvm::Value* codegenGpuKeepPointerFlat(llvm::Value* val) {
  ...
  llvm::InlineAsm* identity = llvm::InlineAsm::get(asmTy,
                                                   /*AsmString=*/"",
                                                   /*Constraints=*/"=v,0",
...
codegenLoadLLVM(llvm::Value* ptr,
                Type* valType,
                Type* surroundingStruct = NULL,
                uint64_t fieldOffset = 0,
                llvm::MDNode* fieldTbaaTypeDescriptor = NULL,
  ...
  if (USE_TBAA && valType &&
    ...
    if (surroundingStruct) {
      ...
      if (isConst)
        tbaa = valType->symbol->llvmConstTbaaAccessTag;
      else
        tbaa = valType->symbol->llvmTbaaAccessTag;
    }
  }
...
static llvm::LoadInst*
codegenLoadLLVM(GenRet ptr, Type* valType = NULL, bool isConst = false) {
  if (ptr.chplType && !valType) {
    if (ptr.isLVPtr)
      valType = ptr.chplType;
    else
  ...
  return codegenLoadLLVM(ptr.val,
                         valType,
                         ptr.surroundingStruct,
                         ptr.fieldOffset,
                         ptr.fieldTbaaTypeDescriptor,
  ...
  if (info->cfile) {
    ...
    if (!ret.val)
      INT_FATAL("Could not find global %s, "
                "perhaps it is missing or is complicated macro?",
                global);
...
static GenRet codegenLocaleForNode(GenRet node) {
  ...
  GenRet ret = codegenCallExprWithArgs(baseBuildLocaleId,
                                       args,
...
static GenRet codegenUseCid(Type* classType) {
  std::string varname;
...
static GenRet codegenGetNodeID(void) {
...
static GenRet codegenUseGlobal(std::string str) {
  return codegenUseGlobal(str.c_str());
}

static GenRet codegenWideHere(GenRet addr, Type* wideType = NULL) {
...
static Type* getRefTypesForWideThing(GenRet wide, Type** wideRefTypeOut) {
  Type* ret = NULL;
  ...
  if (wide.chplType) {
    ...
    if (wide.isLVPtr == GEN_WIDE_PTR) {
      ...
      if (wide.chplType->symbol->hasEitherFlag(FLAG_WIDE_REF,
                                               FLAG_WIDE_CLASS)) {
...
static GenRet codegenWideThingField(GenRet ws, WideThingField field) {
  ...
  INT_ASSERT(field == WIDE_GEP_LOC || field == WIDE_GEP_ADDR);
...
GenRet codegenRaddr(GenRet wide) {
  ...
  if (!fLLVMWideOpt) {
    ...
    llvm::Function* fn =
      getAddrFn(info->module, &info->globalToWideInfo, addrType);
    INT_ASSERT(fn);
...
static GenRet codegenRlocale(GenRet wide) {
  ...
  if (!fLLVMWideOpt) {
    ...
    llvm::Function* fn =
      getLocFn(info->module, &info->globalToWideInfo, addrType);
    INT_ASSERT(fn);
    ret.val = info->irBuilder->CreateCall(fn, wide.val);
    trackLLVMValue(ret.val);
#endif
...
static GenRet codegenRsubloc(GenRet wide) {
  ...
  if (!fLLVMWideOpt) {
    ret =
      codegenCallExpr("chpl_sublocFromLocaleID",
                      codegenValue(codegenWideThingField(wide, WIDE_GEP_LOC)),
                      /*ln*/ codegenZero(),
                      /*fn*/ codegenZero32());
  } else {
...
static GenRet codegenRnode(GenRet wide) {
  ...
  if (!fLLVMWideOpt) {
    ret =
      codegenCallExpr("chpl_nodeFromLocaleID",
                      codegenValue(codegenWideThingField(wide, WIDE_GEP_LOC)),
                      /*ln*/ codegenZero(),
                      /*fn*/ codegenZero32());
  } else {
    ...
    llvm::Function* fn =
      getNodeFn(info->module, &info->globalToWideInfo, addrType);
    INT_ASSERT(fn);
    ret.val = info->irBuilder->CreateCall(fn, wide.val);
    trackLLVMValue(ret.val);
#endif
  ...
  if (info->cfile) {
    ...
    if (isUnion(ct) && !ct->symbol->hasFlag(FLAG_EXTERN) && !special) {
      ...
      if (isCArrayField &&
        ...
        INT_ASSERT(retTy);
        ret.val = info->irBuilder->CreateStructGEP(
          baseTy, baseValue, fieldno, c_field_name);
        trackLLVMValue(ret.val);
        llvm::StructType* sTy = llvm::cast<llvm::StructType>(baseTy);
        INT_ASSERT(sTy);
        llvm::Type* eltTy = sTy->getElementType(fieldno);
        ret.val =
          info->irBuilder->CreateStructGEP(eltTy, ret.val, 0, c_field_name);
        ret.isLVPtr = GEN_VAL;
        trackLLVMValue(ret.val);
      } else {
        ret.val = info->irBuilder->CreateStructGEP(
    ...
    if (base.aliasScope) ret.aliasScope = base.aliasScope;
    if (base.noalias) ret.noalias = base.noalias;
...
static GenRet codegenFieldPtr(GenRet base, Expr* field) {
...
//
static GenRet
codegenElementPtr(GenRet base, GenRet index, bool ddataPtr = false) {
  GenRet ret;
  GenInfo* info = gGenInfo;
  Type* baseType = NULL;
  ...
  if (info->cfile) {
    ...
    if (isStarTuple) {
      GEPLocs.push_back(llvm::Constant::getNullValue(
...
GenRet codegenValuePtr(GenRet r) {
  ...
  if (ret.isLVPtr == GEN_PTR && r.chplType != dtNil) return ret;
...
static GenRet codegenLocalAddrOf(GenRet r) {
  if (r.isLVPtr == GEN_WIDE_PTR) {
...
static GenRet codegenEquals(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
...
static GenRet codegenLessEquals(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
  ...
  if (info->cfile)
    ...
    PromotedPair values = convertValuesToLarger(
      av.val, bv.val, isSignedType(av.chplType), isSignedType(bv.chplType));
...
static GenRet codegenLogicalOr(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
...
#ifdef HAVE_LLVM
    ret.val =
      info->irBuilder->CreateOr(info->irBuilder->CreateIsNotNull(av.val),
...
static GenRet codegenLogicalAnd(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
...
#ifdef HAVE_LLVM
    ret.val =
      info->irBuilder->CreateAnd(info->irBuilder->CreateIsNotNull(av.val),
                                 info->irBuilder->CreateIsNotNull(bv.val));
    trackLLVMValue(ret.val);
#endif
  }
...
static GenRet codegenAdd(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
  ...
  if (info->cfile)
    ...
    if (av.chplType == dtComplex[COMPLEX_SIZE_64]) {
      ...
      ret.val = createInBoundsGEP(
        genEltType.type, ptr->val, extendToPointerSize(*i, AS));
    } else {
      PromotedPair values =
      ...
      if (values.a->getType()->isFPOrFPVectorTy()) {
        ...
        ret.val = info->irBuilder->CreateAdd(
          values.a, values.b, "", false, values.isSigned);
        trackLLVMValue(ret.val);
      }
...
static GenRet codegenSub(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
  ...
  if (info->cfile)
    ...
    if (av.chplType == dtComplex[COMPLEX_SIZE_64]) {
      ...
      if (values.a->getType()->isFPOrFPVectorTy()) {
        ...
        ret.val = info->irBuilder->CreateSub(
          values.a, values.b, "", false, values.isSigned);
        trackLLVMValue(ret.val);
      }
      ret.isUnsigned = !values.isSigned;
...
static GenRet codegenNeg(GenRet a) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  GenRet av = codegenValue(a);
...
#else
      ret.val =
        info->irBuilder->CreateNeg(value, "", /*NUW*/ false, /*NSW*/ av_signed);
#endif
      trackLLVMValue(ret.val);
    }
...
static GenRet codegenMul(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
  ...
  if (info->cfile)
    ...
    if (av.chplType == dtComplex[COMPLEX_SIZE_64]) {
      ...
      if (values.a->getType()->isFPOrFPVectorTy()) {
        ...
        ret.val = info->irBuilder->CreateMul(
          values.a, values.b, "", false, values.isSigned);
        trackLLVMValue(ret.val);
      }
      ret.isUnsigned = !values.isSigned;
...
static GenRet codegenDiv(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
...
static GenRet codegenMod(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
...
#ifdef HAVE_LLVM
    llvm::Value* amt =
      convertValueToType(bv.val, av.val->getType(), isSignedType(bv.chplType));
    bool av_signed = false;
    if (av.chplType) av_signed = isSignedType(av.chplType);
...
static GenRet codegenXor(GenRet a, GenRet b) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (a.chplType && a.chplType->symbol->isRefOrWideRef()) a = codegenDeref(a);
  if (b.chplType && b.chplType->symbol->isRefOrWideRef()) b = codegenDeref(b);
...
static GenRet codegenTernary(GenRet cond, GenRet ifTrue, GenRet ifFalse) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  ...
  if (info->cfile) {
    ...
    llvm::BasicBlock* blockEnd =
      llvm::BasicBlock::Create(info->module->getContext(), "ternaryBlockEnd");
    ...
    llvm::Value* tmp =
      createVarLLVM(values.a->getType(), ret.chplType, (Symbol*)nullptr, name);
    ...
    ret.val =
      info->irBuilder->CreateLoad(ifTrue.chplType->symbol->getLLVMType(), tmp);
    trackLLVMValue(ret.val);
...
// AKA == null
static GenRet codegenIsZero(GenRet x) {
  GenInfo* info = gGenInfo;
  GenRet ret;
...
// AKA != null
static GenRet codegenIsNotZero(GenRet x) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  if (x.chplType->symbol->hasEitherFlag(FLAG_WIDE_REF, FLAG_WIDE_CLASS)) {
    x = codegenRaddr(x);
...
static GenRet codegenGlobalArrayElement(const char* table_name,
...
static GenRet codegenDynamicCastCheck(GenRet cid_Td, Type* C) {
  ...
  GenRet n2_C =
    codegenGlobalArrayElement("chpl_subclass_max_id", "chpl__class_id", cid_C);
...
static GenRet codegenArgForFormal(GenRet arg,
  ...
  if (formal) {
    ...
    } else if (argRequiresCPtr(formal) || formal->qualType().isRef() ||
...
static void codegenCallExprInnerCoerceError(FnSymbol* fn,
  ...
  if (fn && formal)
    USR_FATAL_CONT(
      fn, "mismatched type for '%s' when calling '%s'", formal->name, fn->name);
  else if (func)
    USR_FATAL_CONT("argument to '%s' cannot be passed",
                   func->getName().str().c_str());
  else
    USR_FATAL_CONT("argument to function cannot be passed");
  if (callLoc.lineno() != 0) USR_PRINT(callLoc, "function called here");
  if (isExternOrExport)
    USR_PRINT("check that the argument types match the C function signature");
  USR_STOP();
}
#endif
...
static GenRet codegenCallExprInner(GenRet function,
  ...
  bool isExternOrExport =
    chplFnType && (chplFnType->isExtern() || chplFnType->isExport());
  ...
  for (size_t i = 0; i < args.size(); i++) {
    ...
    args[i] =
      codegenArgForFormal(args[i], formal, defaultToValues, isExternOrExport);
  }
  ...
  if (info->cfile) {
    ...
    if (CGI == nullptr && fnType != nullptr &&
        fnType->getReturnType()->isVoidTy() && fnType->getNumParams() >= 1 &&
    ...
    if (fn && fn->numFormals() > 0) formal = fn->getFormal(1);
    ...
    for (size_t i = 0; i < args.size(); i++) {
      ...
      if (argInfo) {
        ...
        switch (argInfo->getKind()) {
          case clang::CodeGen::ABIArgInfo::Kind::Ignore: break;
          ...
          case clang::CodeGen::ABIArgInfo::Kind::Direct: {
            ...
            if (!llvm::isa<llvm::StructType>(argInfo->getCoerceToType()) &&
              ...
              if (val == nullptr)
                codegenCallExprInnerCoerceError(
                  fn, callLoc, func, formal, isExternOrExport);
              ...
              if (argInfo->isDirect() && argInfo->getCanBeFlattened() && sTy &&
                ...
                if (unsigned offset = argInfo->getDirectOffset()) {
                  ...
                  trackLLVMValue(ptr);
                  ptr =
                    irBuilder->CreateConstInBoundsGEP1_32(i8PtrTy, ptr, offset);
                  trackLLVMValue(ptr);
                }
                ...
                val = convertValueToType(
                  val, argInfo->getCoerceToType(), !tmp.isUnsigned, true);
                if (val == nullptr)
                  codegenCallExprInnerCoerceError(
                    fn, callLoc, func, formal, isExternOrExport);
                llArgs.push_back(val);
        ...
        if (func && fnType && llArgs.size() < fnType->getNumParams()) {
          bool funcHasAttribute = func->getAttributes().hasAttributeAtIndex(
            llArgs.size() + 1, llvm::Attribute::ByVal);
          if (funcHasAttribute)
      ...
      if (formal) formal = next_formal(formal);
    }
    ...
    if (!fnType->isVarArg() && fnType->getNumParams() != llArgs.size()) {
      if (fn)
        USR_FATAL_CONT(
          fn, "mismatched number of arguments in call to '%s'", fn->name);
      else if (func)
      ...
      if (isExternOrExport)
        USR_PRINT(
          "check that the number of arguments match the C function signature");
      USR_STOP();
    }
    ...
    if (chapelRetTy && ret.val->getType() != chapelRetTy) {
      ret.val = convertValueToType(ret.val, chapelRetTy, chplRetTySigned, true);
    }
...
static GenRet codegenCallExprWithArgs(GenRet function,
  ...
  return codegenCallExprInner(
    function, args, fnSym, callLoc, FD, nullptr, defaultToValues);
}
...
GenRet codegenCallExprWithArgs(const char* fnName,
  ...
  if (info->cfile) {
    fn.c = fnName;
    return codegenCallExprWithArgs(
      fn, args, fnName, fnSym, callLoc, FD, defaultToValues);
  } else {
#ifdef HAVE_LLVM
...
static void codegenCallWithArgs(const char* fnName,
  ...
  GenInfo* info = gGenInfo;
  GenRet r =
    codegenCallExprWithArgs(fnName, args, fnSym, callLoc, FD, defaultToValues);
  if (info->cfile) {
...
static void codegenCall(
  const char* fnName, GenRet a1, GenRet a2, GenRet a3, GenRet a4, GenRet a5) {
  std::vector<GenRet> args;
  args.push_back(a1);
  args.push_back(a2);
  args.push_back(a3);
...
static GenRet codegenZero() { return new_IntSymbol(0, INT_SIZE_64)->codegen(); }
...
static void
codegenCallMemcpy(GenRet dest, GenRet src, GenRet size, Type* pointedToType) {
  GenInfo* info = gGenInfo;
  ...
  if (info->cfile) {
    ...
    llvm::Type* types[3];
    unsigned addrSpaceDest =
      llvm::cast<llvm::PointerType>(dest.val->getType())->getAddressSpace();
    unsigned addrSpaceSrc =
      llvm::cast<llvm::PointerType>(src.val->getType())->getAddressSpace();
    ...
    llvm::Function* func = llvm::Intrinsic::getOrInsertDeclaration(
      info->module, llvm::Intrinsic::memcpy, types);
#else
    ...
    // isVolatile?
    llArgs[3] = llvm::ConstantInt::get(
      llvm::Type::getInt1Ty(info->module->getContext()), 0, false);
...
static llvm::Constant* codegenSizeofLLVM(llvm::Type* type) {
...
static GenRet codegenSizeof(llvm::Type* type) {
  GenRet ret;
...
static GenRet codegenSizeof(const char* name) {
  GenInfo* info = gGenInfo;
  GenRet ret;
  ret.chplType = SIZE_TYPE;
  if (info->cfile) {
...
static GenRet codegenSizeof(Type* t) { return codegenSizeof(t->symbol->cname); }

// dest dest must have isLVPtr set. This function implements
// part of codegenAssign.
static void codegenCopy(GenRet dest, GenRet src, Type* chplType = NULL) {
  assert(dest.isLVPtr);
    ...
    if (chplType && chplType->symbol->llvmTbaaStructCopyNode) {
      ...
      if (chplType && chplType->symbol->hasFlag(FLAG_STAR_TUPLE)) {
        ...
        useMemcpy = true;
                 ...
                 isTypeSizeSmallerThan(info->module->getDataLayout(),
                                       eltTy,
...
static bool isTupleOfTuple(BaseAST* e) {
  return (e->typeInfo()->symbol->hasFlag(FLAG_STAR_TUPLE) &&
          toDefExpr(toAggregateType(e->typeInfo())->fields.head)
            ->sym->type->symbol->hasFlag(FLAG_TUPLE));
}
...
#ifdef HAVE_LLVM
    codegenCall("printf",
                new_CStringSymbol("%s")->codegen(),
                new_CStringSymbol(arg)->codegen());
#endif
  }
}
...
static void codegenAssign(GenRet to_ptr, GenRet from) {
  ...
  if (from.chplType && to_ptr.chplType) {
    ...
    if (ct && ct->symbol->hasEitherFlag(FLAG_WIDE_REF, FLAG_WIDE_CLASS)) {
      ...
      if (valField && valField->getValType() == dtNil) {
        from =
          codegenAddrOf(codegenWideHere(codegenNullPointer(), to_ptr.chplType));
      }
    }
    if (from.chplType == dtNil) {
      if (to_ptr.chplType->symbol->hasEitherFlag(FLAG_WIDE_REF,
                                                 FLAG_WIDE_CLASS)) {
  ...
  if ((to_ptr.isLVPtr != GEN_WIDE_PTR && from.isLVPtr != GEN_WIDE_PTR)) {
    ...
    if (isStarTuple) {
      ...
      if (!fNoTupleCopyOpt && starTupleLength <= tuple_copy_limit &&
          !isTupleOfTuple(type)) {
...
//
static void codegenOpAssign(GenRet a,
                            GenRet b,
                            const char* op,
...
static GenRet codegenCall(CallExpr* call) {
  ...
  bool isCallToExtern =
    (chplFnType && chplFnType->isExtern()) || (fn && fn->hasFlag(FLAG_EXTERN));
...
// LLVM
  ...
  ret =
    codegenCallExprWithArgs(base, args, cname, fn, call->astloc, nullptr, true);
...
GenRet CallExpr::codegen() {
  ...
  bool canGenerate =
    (fn && !fn->hasFlag(FLAG_NO_CODEGEN)) || (this->isIndirectCall());
...
#define CODEGEN_PRIM(NAME, call, ret) codegen##NAME(call, ret);

// to call another primitive's DEFINE_PRIM
#define FORWARD_PRIM(NAME) CODEGEN_PRIM(NAME, call, ret);
...
DEFINE_PRIM(ARRAY_SET_FIRST) { FORWARD_PRIM(ARRAY_SET); }
...
DEFINE_PRIM(NOOP) {}
DEFINE_PRIM(MOVE) { INT_FATAL("Handled in switch"); }

DEFINE_PRIM(DEREF) { codegenIsSpecialPrimitive(NULL, call, ret); }
DEFINE_PRIM(GET_SVEC_MEMBER_VALUE) {
  codegenIsSpecialPrimitive(NULL, call, ret);
}
...
DEFINE_PRIM(ADDR_OF) { FORWARD_PRIM(SET_REFERENCE); }
...
DEFINE_PRIM(REF_TO_STRING) {
  if (call->get(1)->isWideRef() ||
    ...
    ret = codegenCallExpr(
      "chpl_wideRefToString", codegenRnode(wide), codegenRaddr(wide));
  } else {
...
DEFINE_PRIM(RETURN) {
  ...
  if (!returnVoid) {
    ...
    if (!call->typeInfo()->symbol->isRefOrWideRef() &&
      ...
      switch (returnInfo->getKind()) {
        ...
        case clang::CodeGen::ABIArgInfo::Kind::InAlloca: {
          ...
          if (returnInfo->getInAllocaSRet()) {
            ...
            llvm::Value* v = irBuilder->CreateAlignedLoad(
              sret->getType(), sret, align, "sret");
            trackLLVMValue(v);
        ...
        case clang::CodeGen::ABIArgInfo::Kind::Indirect: {
          ...
          if (returnInfo->isSRetAfterThis()) ++ii;
...
DEFINE_PRIM(UNARY_MINUS) { ret = codegenNeg(call->get(1)); }
...
DEFINE_PRIM(UNARY_LNOT) { ret = codegenIsZero(call->get(1)); }
DEFINE_PRIM(SQRT) { ret = codegenSqrt(call->get(1)); }
DEFINE_PRIM(ABS) { ret = codegenAbs(call->get(1)); }
DEFINE_PRIM(ADD) { ret = codegenAdd(call->get(1), call->get(2)); }
DEFINE_PRIM(SUBTRACT) { ret = codegenSub(call->get(1), call->get(2)); }
DEFINE_PRIM(MULT) { ret = codegenMul(call->get(1), call->get(2)); }
DEFINE_PRIM(DIV) { ret = codegenDiv(call->get(1), call->get(2)); }
DEFINE_PRIM(MOD) { ret = codegenMod(call->get(1), call->get(2)); }
DEFINE_PRIM(FMA) { ret = codegenFma(call->get(1), call->get(2), call->get(3)); }
DEFINE_PRIM(LSH) { ret = codegenLsh(call->get(1), call->get(2)); }
DEFINE_PRIM(RSH) { ret = codegenRsh(call->get(1), call->get(2)); }
DEFINE_PRIM(PTR_EQUAL) { FORWARD_PRIM(EQUAL); }
DEFINE_PRIM(EQUAL) {
...
DEFINE_PRIM(PTR_NOTEQUAL) { FORWARD_PRIM(NOTEQUAL); }
...
DEFINE_PRIM(AND) { ret = codegenAnd(call->get(1), call->get(2)); }
DEFINE_PRIM(OR) { ret = codegenOr(call->get(1), call->get(2)); }
DEFINE_PRIM(XOR) { ret = codegenXor(call->get(1), call->get(2)); }
...
DEFINE_PRIM(ASSIGN) {
  ...
  if (!lg.chplType) lg.chplType = lhsTypeSym->type;
  if (!rg.chplType) rg.chplType = rhsTypeSym->type;
...
DEFINE_PRIM(UNORDERED_ASSIGN) {
  ...
  if (!lhsWide && rhsWide) {
    ...
    if (dstRef) dst = codegenDeref(dst);
    ...
    if (srcRef) src = codegenDeref(src);
...
DEFINE_PRIM(MAX) {
  ...
  if (is_arithmetic_type(t)) {
    if (isIntType(t)) {
      ...
      ret = codegenCallExpr(
        fname.c_str(), codegenUseGlobal(maxFloat), codegenUseGlobal(maxFloat));
    } else {
...
DEFINE_PRIM(GETCID) {
  ...
  if (call->get(1)->getValType()->typeInfo()->symbol->hasFlag(FLAG_NO_OBJECT) ==
        true &&
      call->get(1)->getValType()->typeInfo()->symbol->hasFlag(
        FLAG_OBJECT_CLASS) == false) {
    INT_ASSERT(0);
  }
...
DEFINE_PRIM(SET_SVEC_MEMBER) {
  ...
  if (call->get(3)->isRefOrWideRef() &&
      !ptr.chplType->symbol->isRefOrWideRef()) {
...
DEFINE_PRIM(CHECK_NIL) {
  ...
  codegenCall("chpl_check_nil", ptr, call->get(2), call->get(3));
}
DEFINE_PRIM(LOCAL_CHECK) {
  ...
  if (lhsType->hasEitherFlag(FLAG_WIDE_REF, FLAG_WIDE_CLASS) == true) {
    ...
    codegenCall(
      "chpl_check_local", codegenRnode(lhs), call->get(3), filename, error);
  }
}
DEFINE_PRIM(IS_LOCAL) {
  ...
  if (lhsType->hasEitherFlag(FLAG_WIDE_REF, FLAG_WIDE_CLASS) == true) {
    ...
    ret = codegenCallExpr("chpl_is_local", codegenRlocale(lhs));
  } else {
    ret = gTrue->codegen();
  }
}

DEFINE_PRIM(GET_SERIAL) { ret = codegenCallExpr("chpl_task_getSerial"); }
...
DEFINE_PRIM(GPU_PID_OFFLOAD) {
  ...
  ret = codegenCallExpr(
    "chpl_gpu_pid_offload", cfg->codegen(), pid->codegen(), size->codegen());
}
...
DEFINE_PRIM(GPU_BLOCK_REDUCE) {
  ...
  ret =
    codegenCallExpr(fnName.c_str(), threadData->codegen(), buffer->codegen());
}
...
DEFINE_PRIM(GPU_REDUCE_WRAPPER) {
  ...
  TypeSymbol* redTypeSym =
    toTypeSymbol(toSymExpr(call->get(curArg++))->symbol());
...
DEFINE_PRIM(GPU_THREADIDX_X) {
  ret = codegenCallExpr("chpl_gpu_getThreadIdxX");
}
DEFINE_PRIM(GPU_THREADIDX_Y) {
  ret = codegenCallExpr("chpl_gpu_getThreadIdxY");
}
DEFINE_PRIM(GPU_THREADIDX_Z) {
  ret = codegenCallExpr("chpl_gpu_getThreadIdxZ");
}
...
DEFINE_PRIM(GPU_ALLOC_SHARED) {
  ...
  llvm::GlobalVariable* sharedArray =
    new llvm::GlobalVariable(*info->module,
                             arrayType,
                             false,
  ...
  llvm::Value* sharedArrayPtr = gGenInfo->irBuilder->CreateBitCast(
    sharedArray, voidPtrType, "sharedArrayPtr");
...
DEFINE_PRIM(GET_REQUESTED_SUBLOC) {
  ret = codegenCallExpr("chpl_task_getRequestedSubloc");
}
...
static void codegenPutGet(CallExpr* call, GenRet& ret) {
  ...
  if (!fLLVMWideOpt) {
    ...
    if (localAddr.isLVPtr == GEN_PTR) localAddr = codegenAddrOf(localAddr);

    if (localAddr.isLVPtr == GEN_WIDE_PTR) localAddr = codegenRaddr(localAddr);

    if (isget) {
      codegenCallMemcpy(localAddr, remoteAddr, size, NULL);
    } else {
      codegenCallMemcpy(remoteAddr, localAddr, size, NULL);
    }
  }
}
DEFINE_PRIM(CHPL_COMM_GET) { codegenPutGet(call, ret); }
DEFINE_PRIM(CHPL_COMM_PUT) { codegenPutGet(call, ret); }
DEFINE_PRIM(CHPL_COMM_ARRAY_GET) { codegenPutGet(call, ret); }
DEFINE_PRIM(CHPL_COMM_ARRAY_PUT) { codegenPutGet(call, ret); }
...
DEFINE_PRIM(CHPL_COMM_PUT_STRD) { codegenPutGetStrd(call, ret); }
DEFINE_PRIM(CHPL_COMM_GET_STRD) { codegenPutGetStrd(call, ret); }
...
DEFINE_PRIM(SIZEOF_BUNDLE) {
  ...
  if (type->symbol->hasFlag(FLAG_WIDE_CLASS) || call->get(1)->isWideRef())
...
DEFINE_PRIM(SIZEOF_DDATA_ELEMENT) {
  ...
  if (type->symbol->hasFlag(FLAG_WIDE_CLASS) == true) {
    size = codegenSizeof(
      getDataClassType(type->getField("addr")->type->symbol)->typeInfo());
  } else {
...
DEFINE_PRIM(CAST) {
  if (call->typeInfo()->symbol->hasFlag(FLAG_WIDE_CLASS) || call->isWideRef()) {

    GenRet tmp = call->get(2);

    ret = codegenWideAddrWithAddr(
      tmp, codegenCast(call->get(1)->typeInfo(), codegenRaddr(tmp)));
  } else {
    ...
    if (dst == src && !(isIntType(dst) || isUIntType(dst) || isRealType(dst))) {
      ret = srcGen;
...
static void codegenCallMemset(GenRet dest, Type* type) {
  GenInfo* info = gGenInfo;
...
DEFINE_PRIM(REGISTER_GLOBAL_VAR) {
  ...
  if (fLLVMWideOpt) {
    ...
    llvm::Function* fn = getGlobalToWideFn(
      gGenInfo->module, &gGenInfo->globalToWideInfo, ptr_wide_ptr_ty);
  ...
  codegenCall(
    "chpl_registerGlobalVar", idx, codegenCast("ptr_wide_ptr_t", ptr_wide_ptr));
}
...
DEFINE_PRIM(STRING_COPY) {
  ...
  if (call->get(1)->typeInfo()->symbol->hasFlag(FLAG_WIDE_CLASS)) {
    ...
    ret = codegenCallExpr(
      "chpl_wide_string_copy", cpyFrom, call->get(2), call->get(3));
  } else
    ret = call->codegenBasicPrimitiveExpr();
}
...
DEFINE_PRIM(ASSERT_ON_GPU) { ret = call->codegenBasicPrimitiveExpr(); }
...
DEFINE_PRIM(FTABLE_CALL) {
  ...
  if (gGenInfo->cfile) {
    ...
    if (argMustUseCPtr(arg.chplType)) str += "*";

    str += ",";
    str += call->get(3)->typeInfo()->symbol->cname;

    if (argMustUseCPtr(call->get(3)->typeInfo())) str += "*";
    ...
    GEPLocs[1] = index.val;
    fnPtrPtr = createInBoundsGEP(global->getValueType(), ftable.val, GEPLocs);
    ...
    if (argMustUseCPtr(call->get(2)->typeInfo())) argt = getPointerType(argt);

    argumentTypes.push_back(argt);

    argt = call->get(3)->typeInfo()->codegen().type;

    if (argMustUseCPtr(call->get(3)->typeInfo())) argt = getPointerType(argt);
    ...
    fngen.val = gGenInfo->irBuilder->CreateBitCast(fnPtr, getPointerType(fnTy));
  ...
  if (argMustUseCPtr(call->get(3)->typeInfo())) arg = codegenLocalAddrOf(arg);

  args.push_back(arg);

  ret = codegenCallExprInner(
    fngen, args, nullptr, astlocT::unknownLoc(), nullptr, fnTy, true);
}
DEFINE_PRIM(VIRTUAL_METHOD_CALL) {
  ...
  for_formals(arg, fn) { args.push_back(call->get(i++)); }

  ret =
    codegenCallExprInner(fngen, args, fn, call->astloc, nullptr, nullptr, true);
}
...
#ifdef HAVE_LLVM
static llvm::MDNode* createMetadataScope(llvm::LLVMContext& ctx,
                                         llvm::MDNode* domain,
                                         const char* name) {
...
DEFINE_PRIM(NO_ALIAS_SET) {
  ...
  if (info->cfile) {
    ...
    if (scopeList == NULL) scopeList = llvm::MDNode::get(ctx, scope);
    ...
    if (noAliasList == NULL) {
      ...
      for_actuals(actual, call) {
        if (!first) {
          ...
          if (otherScope == NULL)
            otherScope =
              createMetadataScope(ctx, info->noAliasDomain, noAliasSym->name);
...
DEFINE_PRIM(CONST_ARG_HASH) {
  ...
  if (call->get(1)->isWideRef()) {
    ...
    // primitive
    ret = codegenCallExpr(
      "const_arg_hash", addr, codegenSizeof(sym->typeInfo()->getValType()));
  } else {
    GenRet ptr = codegenValue(arg);
    ...
    // primitive
    ret =
      codegenCallExpr("const_arg_hash",
                      ptr,
...
DEFINE_PRIM(CHECK_CONST_ARG_HASH) {
  ...
  codegenCall("check_const_hash_matches",
              call->get(1),
              call->get(2),
              call->get(3),
              call->get(4),
...
GenRet CallExpr::codegenPrimitive() {
  ...
  if (gGenInfo->cfile && getStmtExpr() && getStmtExpr() == this &&
      ret.c.length() > 0)
    gGenInfo->cStatements.push_back(ret.c + ";\n");
...
GenRet CallExpr::codegenPrimMove() {
  ...
  if (get(1)->typeInfo() == dtNothing) {
    ...
    if (rhsCe->isPrimitive(PRIM_GET_REAL) ||
        rhsCe->isPrimitive(PRIM_GET_IMAG)) {
  ...
  } else if (get(1)->isWideRef() == true && get(2)->isRef() == true) {
    GenRet from = codegenAddrOf(codegenWideHere(get(2)));
    codegenAssign(get(1), from);
    mustPointOutsideOrderIndependentLoop =
      from.mustPointOutsideOrderIndependentLoop;

  } else if (get(1)->isWideRef() == true && get(2)->isWideRef() == false &&
             get(2)->isRef() == false) {
  ...
  } else if (get(1)->isRef() == true && get(2)->isWideRef() == true) {
  ...
  } else if (get(1)->isRef() == true && get(2)->isRef() == false) {
    GenRet from = get(2);
...
static bool codegenIsSpecialPrimitive(BaseAST* target, Expr* e, GenRet& ret) {
  ...
  if (call->id == breakOnCodegenID) debuggerBreakHere();
    ...
    switch (call->primitive->tag) {
      ...
      case PRIM_GET_IMAG: {
        ...
        if (call->get(1)->isWideRef()) {
          ...
          ret = codegenWideAddr(codegenRlocale(call->get(1)), codegenDeref(t2));

        } else {
          ret = codegen_prim_get_real(
            call->get(1), call->get(1)->typeInfo(), isReal);
        }
      ...
      case PRIM_DEREF: {
        ...
        if (call->get(1)->isWideRef() ||
          ...
          ret = codegenDeref(call->get(1));
                   ...
                   target->typeInfo()->symbol->hasFlag(FLAG_STAR_TUPLE)) {
      ...
      case PRIM_GET_MEMBER: {
        ...
        if (call->get(1)->typeInfo()->symbol->hasFlag(FLAG_WIDE_CLASS) ||
          ...
          retval = true;
                   ...
                   ((target->isRef() && call->get(2)->isRef()) ||
                    (target->isWideRef() && call->get(2)->isWideRef()))) {
          ret = codegenFieldPtr(obj, se);
          retval = true;

        } else if (target &&
                   (target->getValType() != call->get(2)->typeInfo())) {
...
GenRet CallExpr::codegenBasicPrimitiveExpr() const {
  ...
  for_alist(actual, argList) {
    ...
    if (actual->isRef() || actual->isWideRef()) gen = codegenDeref(gen);
